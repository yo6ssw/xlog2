...
// Kotlin feeds paddle contacts (setDit/setDah, from the USB HID reader) and
// gets status/transmit callbacks. Mirrors how the desktop shells drive paddle_.
//
// onStatus/onTransmit fire on the keyer's native worker thread; a thread_local
// attach helper attaches that thread to the JVM and detaches it cleanly on
// exit.
...
// Attaches the calling native thread to the JVM on first use and detaches it
// when the thread exits (thread_local destructor). A JVM-owned thread (a Java
// caller) is left alone — GetEnv succeeds, so we never detach it.
struct JniAttach {
  JNIEnv* env = nullptr;
  bool attached = false;
  JNIEnv* get() {
    if (env != nullptr) return env;
    if (g_paddleVm->GetEnv(reinterpret_cast<void**>(&env), JNI_VERSION_1_6) ==
        JNI_OK)
      return env;
  ...
  ~JniAttach() {
    if (attached && g_paddleVm != nullptr) g_paddleVm->DetachCurrentThread();
  }
};
...
JNIEXPORT jlong JNICALL Java_ro_scripca_xlog2_PaddleKeyer_nativeCreate(
    JNIEnv* env, jobject, jobject listener) {
  if (g_paddleVm == nullptr) env->GetJavaVM(&g_paddleVm);
...
JNIEXPORT void JNICALL Java_ro_scripca_xlog2_PaddleKeyer_nativeStart(
    JNIEnv* env, jobject, jlong handle, jstring host, jint port, jint wpm,
    jboolean iambicB, jboolean ultimatic, jboolean autospace, jboolean sidetone,
    jint toneHz, jint level, jint muteTailMs) {
  auto* h = reinterpret_cast<PaddleHolder*>(handle);
  if (h == nullptr) return;
  RemotePaddleConfig pc;
  const char* c = env->GetStringUTFChars(host, nullptr);
  pc.host = (c != nullptr) ? c : "";
  if (c != nullptr) env->ReleaseStringUTFChars(host, c);
  pc.port = port;
  pc.wpm = wpm;
  pc.iambicB = iambicB;
...
Java_ro_scripca_xlog2_PaddleKeyer_nativeStop(JNIEnv*, jobject, jlong handle) {
  auto* h = reinterpret_cast<PaddleHolder*>(handle);
  if (h != nullptr) h->keyer.stop();
}

JNIEXPORT void JNICALL Java_ro_scripca_xlog2_PaddleKeyer_nativeSetDit(
    JNIEnv*, jobject, jlong handle, jboolean p) {
  auto* h = reinterpret_cast<PaddleHolder*>(handle);
  if (h != nullptr) h->keyer.setDit(p);
}

JNIEXPORT void JNICALL Java_ro_scripca_xlog2_PaddleKeyer_nativeSetDah(
    JNIEnv*, jobject, jlong handle, jboolean p) {
  auto* h = reinterpret_cast<PaddleHolder*>(handle);
  if (h != nullptr) h->keyer.setDah(p);
}

JNIEXPORT void JNICALL Java_ro_scripca_xlog2_PaddleKeyer_nativeDestroy(
    JNIEnv* env, jobject, jlong handle) {
  auto* h = reinterpret_cast<PaddleHolder*>(handle);
  if (h == nullptr) return;
  h->keyer.stop();  // joins the worker before we drop the listener
  if (h->listener != nullptr) env->DeleteGlobalRef(h->listener);
  delete h;
}
