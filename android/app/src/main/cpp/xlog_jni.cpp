...
// behind a thread-safe core-thread dispatcher, so all logbook + coordinator
// work stays single-threaded and off the mesh IO thread. The UI drives it
// through the Kotlin `XlogCore` class; results come back via four listener
// callbacks.
//
// QSO rows cross JNI as a single string: fields joined by US (0x1f), rows by RS
// (0x1e), in the fixed order documented in qsoToWire() below. This keeps the
// JNI surface tiny (a handful of String methods) instead of dozens of field
// getters.
...
class MobileCore : public ILogPageView {
 public:
  ...
  std::string localId() {
    std::string s;
    runOnCore([&] { s = sync_.localId(); });
    return s;
  }
  std::string identityKey() {
    std::string s;
    runOnCore([&] { s = sync_.identityKey(); });
    return s;
  }
  int memberCount() {
    int n = 0;
    runOnCore([&] { n = sync_.memberCount(); });
    return n;
  }
  ...
  std::string peerSnapshot() {
    ...
    runOnCore([&] {
      ...
      for (const auto& p : coord_.peerSnapshot()) {
        if (!first) out += RS;
        out += p.id + US + p.name + US + (p.trusted ? "1" : "0") + US +
               (p.online ? "1" : "0") + US + (p.ready ? "1" : "0");
        first = false;
      }
    });
    return out;
  ...
  void trustPeer(const std::string& id) {
    runOnCore([&] { coord_.trustPeer(id); });
  }
  void revokePeer(const std::string& id) {
    runOnCore([&] { coord_.revokePeer(id); });
  }
  void syncNow() {
    runOnCore([&] { coord_.syncNow(); });
  }
 ...
 private:
  ...
  void coreLoop() {
    ...
    page_.onLocalDelete = [this](const std::string& uuid,
                                 const std::string& at) {
    ...
    qrz_.onResult = [this](const QrzResult& r, const std::string& err) {
      std::string w = r.call + US + r.name + US + r.qth + US + r.locator + US +
                      r.country + US + err;
      callS(mQrz_, w);
    };
#endif  // XLOG_NO_QRZ
    ...
    while (running_.load()) disp_.drain(std::chrono::milliseconds(200));
  ...
  jmethodID mLogbook_ = nullptr, mPeers_ = nullptr, mStatus_ = nullptr,
            mQrz_ = nullptr;
};
...
//             trustEnforce, trustedIds (RS-separated), requireIdentity) ->
//             handle
JNIEXPORT jlong JNICALL XC(nativeStart)(
    JNIEnv* env, jobject /*thiz*/, jobject listener, jstring dataDir,
    jstring secret, jstring nodeName, jstring ctyPath, jstring qrzUser,
    jstring qrzPass, jint qrzCacheDays, jstring staticPeers, jint syncPort,
    jboolean trustEnforce, jstring trustedIds, jboolean requireIdentity) {
...
JNIEXPORT jlong JNICALL XC(nativeAddQso)(JNIEnv* env, jobject, jlong h,
                                         jstring wire) {
  return core(h)->addQso(jstr(env, wire));
}
JNIEXPORT void JNICALL XC(nativeUpdateQso)(JNIEnv* env, jobject, jlong h,
                                           jlong id, jstring wire) {
  core(h)->updateQso(id, jstr(env, wire));
}
JNIEXPORT void JNICALL XC(nativeDeleteQso)(JNIEnv*, jobject, jlong h,
                                           jlong id) {
  core(h)->deleteQso(id);
}
JNIEXPORT jstring JNICALL XC(nativeDeriveDxcc)(JNIEnv* env, jobject, jlong h,
                                               jstring call) {
  return toJ(env, core(h)->deriveDxcc(jstr(env, call)));
}
JNIEXPORT jstring JNICALL XC(nativeBandForFreq)(JNIEnv* env, jobject, jlong h,
                                                jdouble mhz) {
  return toJ(env, core(h)->bandForFreq(mhz));
}
JNIEXPORT jstring JNICALL XC(nativeFindDupe)(JNIEnv* env, jobject, jlong h,
                                             jstring wire, jlong editId) {
  return toJ(env, core(h)->findDupe(jstr(env, wire), editId));
}
JNIEXPORT jint JNICALL XC(nativeImportAdif)(JNIEnv* env, jobject, jlong h,
                                            jstring text) {
...
JNIEXPORT void JNICALL XC(nativeLookup)(JNIEnv* env, jobject, jlong h,
                                        jstring call) {
...
JNIEXPORT jstring JNICALL XC(nativePeerSnapshot)(JNIEnv* env, jobject,
                                                 jlong h) {
  return toJ(env, core(h)->peerSnapshot());
}
JNIEXPORT void JNICALL XC(nativeTrustPeer)(JNIEnv* env, jobject, jlong h,
                                           jstring id) {
  core(h)->trustPeer(jstr(env, id));
}
JNIEXPORT void JNICALL XC(nativeRevokePeer)(JNIEnv* env, jobject, jlong h,
                                            jstring id) {
