...
// Kotlin OpusDecoder. The rig-audio stream client (AudioStreamClient.kt) does
// the UDP receive + AudioTrack playback in Kotlin (like QRZ moved to OkHttp)
// and only crosses JNI to decode each Opus packet — so the native surface stays
// tiny and the desktop's PipeWire-coupled AudioStreamClient stays untouched.
//
...
JNIEXPORT jlong JNICALL Java_ro_scripca_xlog2_OpusDecoder_nativeCreate(
...
JNIEXPORT jint JNICALL Java_ro_scripca_xlog2_OpusDecoder_nativeDecode(
    JNIEnv* env, jobject, jlong handle, jbyteArray packet, jint packetLen,
  ...
  const int n =
      opus_decode(dec, reinterpret_cast<const unsigned char*>(pkt), packetLen,
...
JNIEXPORT jint JNICALL Java_ro_scripca_xlog2_OpusDecoder_nativeDecodePlc(
    JNIEnv* env, jobject, jlong handle, jshortArray pcm, jint frameSize) {
  auto* dec = reinterpret_cast<OpusDecoder*>(handle);
  if (dec == nullptr) return -1;
  jshort* out = env->GetShortArrayElements(pcm, nullptr);
  const int n = opus_decode(dec, nullptr, 0, reinterpret_cast<opus_int16*>(out),
                            frameSize, 0);
  env->ReleaseShortArrayElements(pcm, out, 0);
  return n;
}

JNIEXPORT void JNICALL Java_ro_scripca_xlog2_OpusDecoder_nativeDestroy(
