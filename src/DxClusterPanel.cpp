...
DxClusterPanel::DxClusterPanel() : Gtk::Box(Gtk::Orientation::VERTICAL) {
  ...
  connectButton_.signal_clicked().connect(
      [this]() { signalConnectToggle_.emit(); });
  controls->append(connectButton_);

  commandEntry_.set_hexpand(true);
  commandEntry_.set_placeholder_text(
      "cluster command (e.g. sh/dx 20, set/filter)…");
  commandEntry_.signal_activate().connect(
      sigc::mem_fun(*this, &DxClusterPanel::onSendCommand));
  controls->append(commandEntry_);
  auto* send = Gtk::make_managed<Gtk::Button>("Send");
  send->signal_clicked().connect(
      sigc::mem_fun(*this, &DxClusterPanel::onSendCommand));
  ...
  columnView_.signal_activate().connect(
      sigc::mem_fun(*this, &DxClusterPanel::onActivate));

  columnView_.append_column(makeColumn(
      "Freq", [](const BandMapItem& r) { return formatKHz(r.freqKHz); }));
  columnView_.append_column(
      makeColumn("DX", [](const BandMapItem& r) { return r.dxCall; }));
  columnView_.append_column(
      makeColumn("Band", [](const BandMapItem& r) { return r.band; }));
  columnView_.append_column(makeCountColumn());
  columnView_.append_column(makeColumn(
      "Entity", [](const BandMapItem& r) { return r.entity; }, true));
  columnView_.append_column(
      makeColumn("Cont", [](const BandMapItem& r) { return r.continent; }));

  auto* spotScroller = Gtk::make_managed<Gtk::ScrolledWindow>();
  spotScroller->set_policy(Gtk::PolicyType::AUTOMATIC,
  ...
  consoleScroller->set_policy(Gtk::PolicyType::AUTOMATIC,
                              Gtk::PolicyType::AUTOMATIC);
  ...
  // without new traffic.
  expiryTimer_ = Glib::signal_timeout().connect(
      [this]() {
        rebuild();
        return true;
      },
...
DxClusterPanel::~DxClusterPanel() { expiryTimer_.disconnect(); }

Glib::RefPtr<Gtk::ColumnViewColumn> DxClusterPanel::makeColumn(
    const Glib::ustring& title,
    std::function<std::string(const BandMapItem&)> getter, bool expand) {
...
Glib::RefPtr<Gtk::ColumnViewColumn> DxClusterPanel::makeCountColumn() {
  ...
  factory->signal_bind().connect([conns](
...
bool DxClusterPanel::spotMatchesFilter(
    const Glib::RefPtr<Glib::ObjectBase>& obj) {
  if (activeBands_.empty()) return true;
...
void DxClusterPanel::addSpot(const DxSpot& spot) {
  if (spot.dxCall.empty() || spot.freqKHz <= 0.0) return;
...
void DxClusterPanel::rebuild() {
  ...
  for (auto& [key, e] : entries_) {
    ...
    if (spotters.empty()) expired.push_back(key);
  }
  ...
  for (const auto& [key, e] : entries_) {
    ...
    for (const auto& kv : e.spotters) sorted.push_back(&kv);
    ...
    for (const auto* p : sorted) {
      const auto mins =
          std::chrono::duration_cast<std::chrono::minutes>(now - p->second.time)
              .count();
      if (!tip.empty()) tip += '\n';
    ...
    const std::string comment =
        sorted.empty() ? std::string{} : sorted.front()->second.comment;
    const std::string timeUtc =
        sorted.empty() ? std::string{} : sorted.front()->second.timeUtc;
...
void DxClusterPanel::addLine(const std::string& line) {
  ...
  if (buffer->get_line_count() > kMaxLines + 1) {
    auto cut =
        buffer->get_iter_at_line(buffer->get_line_count() - 1 - kMaxLines);
...
void DxClusterPanel::onActivate(guint position) {
  auto* item =
      dynamic_cast<BandMapItem*>(selection_->get_object(position).get());
  if (!item) return;
