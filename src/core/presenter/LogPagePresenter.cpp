...
void LogPagePresenter::refresh() { view_.setRows(logbook_.qsos()); }

// --- view events -------------------------------------------------------------

void LogPagePresenter::onCallChanged() { updateIndicators(); }

void LogPagePresenter::onLocatorChanged() {
  if (onLocator) onLocator(view_.formData().locator);
}
...
void LogPagePresenter::onDupeKeyChanged() { updateIndicators(); }
...
void LogPagePresenter::onDelete() {
  ...
  changed();
  status("QSO deleted.");
  if (rr.ok && onLocalDelete) onLocalDelete(rr.uuid, rr.deleted_at);
}

void LogPagePresenter::onClear() { clearForm(); }
...
void LogPagePresenter::deleteQso(long id) {
  ...
  refreshList();
  if (editingId_ == id) clearForm();  // the deleted row was loaded in the form
  changed();
  status("QSO deleted.");
  if (rr.ok && onLocalDelete) onLocalDelete(rr.uuid, rr.deleted_at);
}
...
bool LogPagePresenter::openFile(const std::string& path) {
  if (!logbook_.open(path)) return false;
...
bool LogPagePresenter::saveAs(const std::string& path) {
  if (!logbook_.saveAs(path)) return false;
  refreshList();
  changed();
  return true;
}
...
void LogPagePresenter::emitUpsert(long id) {
  if (!onLocalUpsert) return;
  if (const Qso* q = findQso(id)) onLocalUpsert(*q);
}
...
bool LogPagePresenter::enrichFromQrz(long id, const QrzResult& r) {
  ...
  if (!found) return false;  // the QSO was deleted before the lookup returned
...
void LogPagePresenter::setRigFrequency(double mhz) {
  ...
  if (view_.formData().freq != f) view_.setFreq(f);
  const std::string b = bands::forFrequencyMHz(mhz);
  if (!b.empty()) view_.setBand(b);
}
...
void LogPagePresenter::setRigMode(const std::string& mode) {
  ...
  if (std::find(modes.begin(), modes.end(), mode) == modes.end()) return;
...
void LogPagePresenter::applyQrzLookup(const QrzResult& r) {
  ...
  if (!r.locator.empty() && f.locator.empty()) f.locator = r.locator;
  view_.setFormData(f);
  ...
  if (onLocator) onLocator(f.locator);
}
...
void LogPagePresenter::applyDxSpot(const std::string& call, double mhz) {
  ...
  status("DX spot: " + strutil::toUpper(call) + " on " + formatMhz(mhz) +
         " MHz.");
}

void LogPagePresenter::backfillDxcc() {
  if (!dxcc::available()) return;
  ...
  refreshList();
  changed();
  for (const auto& u : updates) emitUpsert(u.id);
...
std::vector<std::string> LogPagePresenter::callsignsMissingLocator() const {
  ...
  for (const auto& q : logbook_.qsos()) {
    if (q.call.empty() || !q.locator.empty()) continue;
    const std::string up = strutil::toUpper(q.call);
    if (seen.insert(up).second) calls.push_back(up);
  }
  return calls;
}
...
int LogPagePresenter::applyLocatorFill(
  ...
  for (const auto& q : logbook_.qsos()) {
    ...
    if (it == callToLocator.end() || it->second.empty()) continue;
    Qso u = q;
    u.locator = it->second;
    updates.push_back(std::move(u));
  }
...
void LogPagePresenter::markLotwSent(const std::vector<long>& ids,
                                    const std::string& date) {
  logbook_.markLotwSent(ids, date);
  refreshList();
  changed();
  for (long id : ids) emitUpsert(id);
}

int LogPagePresenter::applyLotwConfirmations(
    const std::vector<Qso>& confirmed) {
