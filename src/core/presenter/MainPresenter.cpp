...
void MainPresenter::routeUdp(const std::vector<Qso>& qsos,
                             const std::string& source) {
  LogPagePresenter* log = view_.currentLog();
  if (!log || qsos.empty()) return;
  ...
  const bool qrzReady =
      !settings.qrzUser.empty() && !settings.qrzPassword.empty();
  for (const auto& q : qsos) {
    ...
    if (qrzReady && !q.call.empty() &&
        (q.name.empty() || q.qth.empty() || q.locator.empty() ||
         q.country.empty()))
...
void MainPresenter::pumpQrzEnrich() {
  while (!qrzEnrichActive_ && !qrzEnrichQueue_.empty()) {
    ...
    if (!view_.startQrzLookup(job.call)) return;
...
void MainPresenter::routeRigUpdate(double mhz, const std::string& mode) {
  ...
  if (mhz == lastRigMhz_ && mode == lastRigMode_) return;
  lastRigMhz_ = mhz;
  lastRigMode_ = mode;
  std::string s = "Rig: " + std::to_string(mhz) + " MHz";
  if (!mode.empty()) s += " " + mode;
  status(s);
}

void MainPresenter::routeLotwDownload(const std::string& adif,
...
void MainPresenter::routeLotwUploadResult(bool ok, const std::string& message) {
  if (ok && pendingUpload_ && view_.isLogLive(pendingUpload_))
    pendingUpload_->markLotwSent(pendingUploadIds_,
                                 timeutil::utcNow("%Y-%m-%d"));
  pendingUpload_ = nullptr;
  pendingUploadIds_.clear();
  status("LoTW upload: " + message);
}

void MainPresenter::routeQrzResult(const QrzResult& result,
                                   const std::string& error) {
  ...
  if (log && view_.isLogLive(log)) log->applyQrzLookup(result);
...
void MainPresenter::routeQrzLocatorFill(
  ...
  if (log && view_.isLogLive(log)) {
    std::map<std::string, std::string> m(callLocators.begin(),
                                         callLocators.end());
...
void MainPresenter::beginLotwUpload(LogPagePresenter* target,
                                    std::vector<long> ids) {
