...
void SyncCoordinator::sendHello(const PeerKey& peer) {
  ...
  transport_.sendTo(
      peer, {Type::Hello, syncproto::encodeHello(nodeId_, synced_->syncId(),
                                                 nonce, secret_)});
...
void SyncCoordinator::onMessage(const PeerKey& peer, const Message& m) {
  ...
  switch (m.type) {
    case Type::Hello:
      ...
      break;
    case Type::HelloAck:
      handleHelloAck(peer, m);
      break;
    case Type::Digest:
      handleDigest(peer, m);
      break;
    case Type::Manifest:
      handleManifest(peer, m);
      break;
    case Type::Request:
      handleRequest(peer, m);
      break;
    case Type::Records:
      handleRecords(peer, m);
      break;
    case Type::Tombstones:
      handleTombstones(peer, m);
      break;
    case Type::Push:
      handlePush(peer, m);
      break;
    default:
      break;  // Ping/Pong unused (the mesh has its own heartbeats)
  }
}
...
void SyncCoordinator::handleHello(const PeerKey& peer, const Message& m) {
  syncproto::Hello h;
  if (!syncproto::decodeHello(m.payload, secret_, h)) return;
  PeerState& st = peers_[peer];
  if (!h.authOk) {
    st.refused = true;
    transport_.sendTo(
        peer, {Type::HelloAck,
               syncproto::encodeHelloAck(false, "", "authentication failed")});
  ...
  if (!localId.empty() && !peerId.empty() && localId != peerId) {
    ...
    transport_.sendTo(
        peer, {Type::HelloAck,
               syncproto::encodeHelloAck(false, localId, "different logbook")});
  ...
  transport_.sendTo(
      peer, {Type::HelloAck, syncproto::encodeHelloAck(true, resolved, "")});
...
void SyncCoordinator::handleHelloAck(const PeerKey& peer, const Message& m) {
  ...
  if (!syncproto::decodeHelloAck(m.payload, a)) return;
  PeerState& st = peers_[peer];
  if (!a.ok) {
    st.refused = true;
...
void SyncCoordinator::handleDigest(const PeerKey& peer, const Message& m) {
  if (peers_[peer].refused) return;
  std::string peerHex;
  if (!syncproto::decodeDigest(m.payload, peerHex)) return;
...
void SyncCoordinator::handleManifest(const PeerKey& peer, const Message& m) {
  ...
  if (!syncproto::decodeManifest(m.payload, peerMan)) return;
  ...
  for (const auto& e : peerMan.records) {
    ...
    if (it == localLive.end() || e.stamp > it->second) pull.push_back(e.uuid);
  }
  ...
  if (!pushUuids.empty())
    transport_.sendTo(
        peer, {Type::Records,
               syncproto::encodeRecords(synced_->recordsByUuids(pushUuids))});
  if (!local.tombstones.empty())
    transport_.sendTo(peer, {Type::Tombstones,
                             syncproto::encodeTombstones(local.tombstones)});
...
void SyncCoordinator::handleRequest(const PeerKey& peer, const Message& m) {
  ...
  if (!syncproto::decodeRequest(m.payload, uuids) || uuids.empty()) return;
  transport_.sendTo(peer, {Type::Records, syncproto::encodeRecords(
                                              synced_->recordsByUuids(uuids))});
}
...
void SyncCoordinator::handleRecords(const PeerKey& peer, const Message& m) {
  ...
  if (st.refused) return;
  std::vector<Qso> recs;
  if (!syncproto::decodeRecords(m.payload, recs) || recs.empty()) return;
  const MergeResult r =
      synced_->applyRemoteDelta(recs, {}, nodeId_, st.peerNodeId);
...
void SyncCoordinator::handleTombstones(const PeerKey& peer, const Message& m) {
  ...
  if (!syncproto::decodeTombstones(m.payload, tombs) || tombs.empty()) return;
  const MergeResult r =
      synced_->applyRemoteDelta({}, tombs, nodeId_, st.peerNodeId);
  if (r.deleted)
    status("Sync: removed " + std::to_string(r.deleted) +
           " QSO(s) deleted on peer.");
}
...
void SyncCoordinator::handlePush(const PeerKey& peer, const Message& m) {
  ...
  if (st.refused) return;
  syncproto::Push p;
  if (!syncproto::decodePush(m.payload, p)) return;
  if (p.isTombstone)
...
void SyncCoordinator::onLocalUpsert(const Qso& q) {
  ...
  for (const auto& [peer, st] : peers_)
    if (st.ready && !st.refused) transport_.sendTo(peer, m);
}

void SyncCoordinator::onLocalDelete(const std::string& uuid,
                                    const std::string& deletedAt) {
  const Message m{Type::Push,
                  syncproto::encodePushTombstone({uuid, deletedAt})};
  for (const auto& [peer, st] : peers_)
    if (st.ready && !st.refused) transport_.sendTo(peer, m);
}
...
void SyncCoordinator::syncNow() {
  ...
  for (const auto& [peer, st] : peers_) {
    if (st.ready && !st.refused) {
      beginAntiEntropy(peer);
      ++n;
    }
  }
  if (n == 0) status("Sync: no connected peers.");
}
