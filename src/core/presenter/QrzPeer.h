...
//    the first record a peer returns, or nullopt after a short timeout / when
//    no peers are connected. Wired as QrzClient's PeerResolver so a lookup
//    consults peers between the local cache and qrz.com.
//
// Because the mesh group is derived from the shared secret, every reachable
// peer already proved knowledge of the secret — so sharing cached callsign data
// across the mesh needs no further gating.
class QrzPeer {
 public:
  QrzPeer(LogbookSync& transport, QrzClient& qrz)
      : transport_(transport), qrz_(qrz) {}
 ...
 private:
  void handleQuery(const LogbookSync::PeerKey& peer,
