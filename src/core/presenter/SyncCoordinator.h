...
// independently per peer. For each peer: HELLO/HELLO_ACK authenticate
// (pre-shared secret HMAC) and agree a shared sync_id; then the higher-node-id
// side initiates a DIGEST; on mismatch the other returns its MANIFEST; the
// initiator pulls what it lacks (REQUEST -> RECORDS) and pushes what it has
// newer (RECORDS), applying the manifest's tombstones. Steady state: a local
// change is pushed (PUSH) to every authenticated peer; the per-peer
// anti-entropy pass repairs anything missed. Every merge is last-write-wins and
// idempotent, so PUSH and anti-entropy compose safely across any number of
// peers and any message ordering.
class SyncCoordinator {
 ...
 private:
  ...
  void handleHelloAck(const LogbookSync::PeerKey& peer,
                      const syncproto::Message&);
  void handleDigest(const LogbookSync::PeerKey& peer,
                    const syncproto::Message&);
  void handleManifest(const LogbookSync::PeerKey& peer,
                      const syncproto::Message&);
  void handleRequest(const LogbookSync::PeerKey& peer,
                     const syncproto::Message&);
  void handleRecords(const LogbookSync::PeerKey& peer,
                     const syncproto::Message&);
  void handleTombstones(const LogbookSync::PeerKey& peer,
                        const syncproto::Message&);
  ...
  void peersChanged() {
    if (onPeersChanged) onPeersChanged();
  }
