...
class MainPresenter {
 public:
  ...
  // A finished bulk locator fill: write the locators back to the requesting
  // tab.
 ...
 private:
  ...
  // Last rig reading shown in the status bar; the rig polls every ~500 ms but
  // we only refresh the status line when the frequency or mode actually
  // changes, so a steady rig doesn't repaint it every tick (and stomp other
  // status messages).
