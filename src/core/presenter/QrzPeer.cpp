...
void QrzPeer::query(const std::string& callsign,
    ...
    transport_.broadcast({Type::QrzQuery, syncproto::encodeQrzQuery(id, callsign)});

    // Resolve to nullopt if no peer has answered by the deadline.
    scheduleOnce(timeoutMs_, [this, id]() {
        auto it = pending_.find(id);
        if (it == pending_.end())
            return;  // already answered by a peer
...
void QrzPeer::onMessage(const LogbookSync::PeerKey& peer, const Message& m) {
    switch (m.type) {
        case Type::QrzQuery:    handleQuery(peer, m);  break;
        case Type::QrzResponse: handleResponse(m);     break;
        default: break;
    }
}
...
void QrzPeer::handleQuery(const LogbookSync::PeerKey& peer, const Message& m) {
    ...
    if (auto cached = qrz_.cachedLookup(callsign); cached && !cached->call.empty()) {
        transport_.sendTo(peer, {Type::QrzResponse, syncproto::encodeQrzResponse(id, *cached)});
...
void QrzPeer::handleResponse(const Message& m) {
    std::uint32_t id = 0;
    QrzResult result;
    if (!syncproto::decodeQrzResponse(m.payload, id, result) || result.call.empty())
        return;
    auto it = pending_.find(id);
    if (it == pending_.end())
