...
bool LotwClient::downloadConfirmations(const std::string& user,
  ...
  std::string url = "https://lotw.arrl.org/lotwuser/lotwreport.adi?login=" +
                    urlEscape(curl, user) +
                    "&password=" + urlEscape(curl, password) +
                    "&qso_query=1&qso_qsl=yes&qso_qsldetail=yes";
  if (!since.empty()) url += "&qso_qslsince=" + urlEscape(curl, since);
...
void LotwClient::deliverDownload() {
  ...
  if (thread_.joinable()) thread_.join();
  busy_.store(false);
  if (onDownloadDone) onDownloadDone(body, error);
}
...
void LotwClient::uploadAdifFile(const std::string& tqslPath,
  ...
  std::vector<std::string> argv = {
      ...
      "-a",
      "all",  // process all QSOs (don't stop on duplicates)
  ...
  uploader_.run(argv, [this, w = std::weak_ptr<bool>(alive_)](
    ...
    if (w.expired() || !onUploadDone) return;
    if (!r.error.empty())
      onUploadDone(false,
                   "Could not run tqsl: " + r.error + " — is tqsl installed?");
    else if (r.ok)
