...
std::vector<std::pair<std::string, std::string>> parseElements(
...
QrzClient::QrzClient(IUiDispatcher& ui) : ui_(ui) { ensureCurlGlobalInit(); }
...
void QrzClient::startNetwork() {
  if (thread_.joinable()) thread_.join();
  thread_ = std::thread(&QrzClient::worker, this, net_.user, net_.password,
                        net_.callsign);
}
...
void QrzClient::cachePut(const QrzResult& result) {
  if (!result.call.empty()) cache_.put(result);
}
...
bool QrzClient::doLogin(void* curlV, const std::string& user,
  ...
  const std::string url =
      std::string(kBase) + "?username=" + urlEscape(curl, user) +
      "&password=" + urlEscape(curl, password) + "&agent=xlog2-0.1";
  std::string e;
...
QrzResult QrzClient::fetchOne(void* curlV, const std::string& user,
  ...
  auto query = [&](std::string& e) -> std::string {
    const std::string url = std::string(kBase) +
                            "?s=" + urlEscape(curl, sessionKey_) +
                            "&callsign=" + urlEscape(curl, callsign);
...
void QrzClient::worker(std::string user, std::string password,
                       std::string callsign) {
  QrzResult result;
  std::string error;
  ...
  if (CURL* curl = curl_easy_init()) {
    ...
    if (error.empty() && !result.call.empty()) cache_.put(result);
  } else {
...
bool QrzClient::fillLocators(const std::string& user,
                             const std::string& password,
  ...
  if (thread_.joinable()) thread_.join();
  thread_ =
      std::thread(&QrzClient::fillWorker, this, user, password, callsigns);
  return true;
}
...
void QrzClient::fillWorker(std::string user, std::string password,
  ...
  if (!curl) {
    ...
    for (const auto& call : callsigns) {
      ...
      if (!r.locator.empty()) out.emplace_back(call, r.locator);
      ++done;
      if (done % 5 == 0 || done == total)
        ui_.post([this, w = std::weak_ptr<bool>(alive_), done, total]() {
          if (!w.expired() && onFillProgress) onFillProgress(done, total);
        });
    }
    curl_easy_cleanup(curl);
  }
...
void QrzClient::deliverFill() {
  ...
  if (thread_.joinable()) thread_.join();
  busy_.store(false);
  if (onFillResult) onFillResult(results, fromCache, fetched, error);
}
