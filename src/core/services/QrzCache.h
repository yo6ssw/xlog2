...
// mutex-guarded so the QrzClient worker thread and the UI thread (configuration)
// can touch it safely.
class QrzCache {
public:
    QrzCache() = default;
    ...
    // Insert or replace the record for its callsign, stamped with the current time.
