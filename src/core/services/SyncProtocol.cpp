...
struct Sha256 {
  ...
  void block(const std::uint8_t* d) {
    ...
    std::uint32_t w[64];
    for (int i = 0; i < 16; ++i)
      w[i] = (std::uint32_t(d[i * 4]) << 24) |
             (std::uint32_t(d[i * 4 + 1]) << 16) |
...
void putQso(std::string& s, const Qso& q) {
  ...
  putStr(s, q.cq_zone);
  putStr(s, q.itu_zone);
  putStr(s, q.continent);
  putStr(s, q.lotw_sent);
  putStr(s, q.lotw_sent_date);
  putStr(s, q.lotw_rcvd);
  putStr(s, q.lotw_rcvd_date);
}
...
Qso getQso(Reader& r) {
  ...
  q.continent = r.str();
  q.lotw_sent = r.str();
  q.lotw_sent_date = r.str();
  q.lotw_rcvd = r.str();
  q.lotw_rcvd_date = r.str();
  return q;
}
...
bool Decoder::next(Message& out) {
  ...
  if (buf_.size() < kHeader + len) return false;  // need more bytes
...
bool decodeHello(const std::string& payload, const std::string& secret,
  ...
  const std::string expect = hmacSha256Hex(
      secret, out.nodeId + "\x1f" + out.syncId + "\x1f" + out.nonce);
