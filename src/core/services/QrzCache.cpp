...
std::vector<std::pair<std::string, std::string>> deserializeFields(
    const std::string& s) {
  ...
  while (i < s.size()) {
    std::string k, v;
    if (!readChunk(k) || !readChunk(v)) break;
...
bool QrzCache::open(const std::string& path) {
  ...
  if (db_ && path == path_) return true;  // already open at this path
  if (db_) {
    sqlite3_close(db_);
    db_ = nullptr;
  }
...
std::optional<QrzResult> QrzCache::get(const std::string& call,
                                       int maxAgeDays) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!db_ || maxAgeDays <= 0) return std::nullopt;
  const std::string key = strutil::toUpper(call);
  sqlite3_stmt* stmt = nullptr;
  const char* sql =
      "SELECT fetched_at,name,qth,locator,country,fields FROM qrz_cache WHERE "
      "call=?";
