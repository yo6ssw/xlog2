...
constexpr uint16_t kVendor = 0x1EAF;  // LeafLabs Maple (the Blue Pill's USB VID)
...
void HidPaddleInput::worker() {
    ...
    while (running_.load()) {
        ...
        if (fds[0].revents & (POLLIN | POLLHUP | POLLERR)) {
            ...
            if (dit != lastDit) { lastDit = dit; if (onDit) onDit(dit); }
            if (dah != lastDah) { lastDah = dah; if (onDah) onDah(dah); }
        }
    }
