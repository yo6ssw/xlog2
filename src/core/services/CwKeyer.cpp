...
bool CwKeyer::setEndpoint(const std::string& host, int port) {
  ...
  if (!ok)
    lastError_ =
        std::string("cannot open keyer socket: ") + std::strerror(errno);
  return ok;
}
...
bool CwKeyer::send(const std::string& text) {
  ...
  if (speed_ > 0 &&
      !sendDatagram(std::string(1, kEsc) + "2" + std::to_string(speed_)))
    return false;
  return sendDatagram(text);
}

bool CwKeyer::abort() { return sendDatagram(std::string(1, kEsc) + "4"); }
