...
std::size_t nextPow2(double v) {
  std::size_t n = 256;
  while (n < static_cast<std::size_t>(v) && n < 4096) n <<= 1;
  return n;
}
...
//   sym       : the same marks hard-classified to '.'/'-' (exact-lookup
//   fallback) dit, dah  : learned dit / dah length references (hops); dah
//   defaults to 2.5x
//
// Order: confident SOM (same length) -> exact string lookup (old behaviour) ->
// edit-distance recovery (length errors). So it is never worse than exact
// lookup.
//
...
// call that is merely absent from the list (e.g. a special-event `RI0SP`) is
// not rewritten into a one-edit list neighbour (`RA0SP`). Only an uncertain
// (length- error-recovered) character is open to substitution.
char decodeChar(const std::vector<float>& dur, const std::string& sym,
  ...
  //    channel is established* (`mature`: it has already cleanly decoded a
  //    couple of characters, so the dit/dah references are settled) — only then
  //    is deferring the per-element call to the character's timing more
  //    reliable than a hard classification. During cold start the references
  //    are unsettled and the SOM would manufacture wrong characters (polluting
  //    the callsign), so it is skipped in favour of exact lookup until the
  //    channel proves real. Accept only when the best fit is good and clearly
  //    ahead of the runner-up, so ambiguous elements are left to abstain rather
  //    than be guessed.
  if (mature && dit > 0.0 && dah > 0.0) {
    double best = 1e9, second = 1e9;
    char bestCh = '\0';
    for (const auto& [pat, ch] : t) {
      if (pat.size() != dur.size()) continue;
      double dist = 0.0;
...
// is only attempted at uncertain positions, so a cleanly-decoded valid call
// that is simply absent from the list (a special-event call) is never rewritten
// into a one-edit list neighbour. Deletions/insertions still run everywhere:
// they repair a token that came out the wrong length (a merged/split element),
// which is inherently a decode error rather than a competing valid call.
std::string lookupCall(const std::string& tok, const std::string& conf,
  ...
  for (std::size_t i = 0; i < tok.size(); ++i) {  // substitutions
    ...
    if (i >= conf.size() || conf[i] != '0') continue;
...
// is an ITU-style prefix (a letter + optional letter/digit, or a digit +
// letter)
...
std::string callsignIn(const std::string& text, const std::string& conf,
  ...
  if (tok.size() < 3 || tok.size() > 16 || !isCallsign(tok)) return {};
  if (db && !db->empty()) {
    const std::string tokConf = start < conf.size()
                                    ? conf.substr(start, end - start + 1)
                                    : std::string{};
...
struct Channel {
  ...
  std::array<bool, kViterbiLag>
      fromOff{};          // OFF-node predecessor was ON? (ring)
  ...
  unsigned __int128 keyHist =
      0;  // last 128 finalised key states (1 bit/frame);
  ...
  double rotRe = 1.0,
         rotIm = 0.0;             // per-sample LO rotation (from the bin freq)
  ...
  double snrFloor =
      0.0;  // aggressively-tracked true noise floor, for the SNR
            // gate only — reaches the real floor so peak/floor is a
            // wide, meaningful SNR (separate from the decode noise)
  double peakPow =
      0.0;  // tracked key-down power; a ghost is an attenuated
  ...
  std::vector<float>
      elemHops;      // raw mark durations (hops) of the char in progress,
...
void appendChar(Channel& ch, char c, bool confident, const CallSet* db) {
  ...
  bool known = false;
  if (std::string call = callsignIn(ch.text, ch.conf, db, known);
      !call.empty()) {
    ch.call = call;
    ch.callKnown = known;
  }
...
void closeSymbol(Channel& ch, const CallSet* db) {
  if (!ch.elemHops.empty()) {
    ...
    if (char c = decodeChar(ch.elemHops, ch.symbol, ch.markUnit, ch.dahUnit,
      ...
      if (ch.elemHops.size() >= 2) ++ch.richChars;
    }
    ch.symbol.clear();
    ch.elemHops.clear();
    ch.dirty = true;
...
// fast (a dit / element gap), leak up slowly (a slow-down). Resists outliers
// and survives a dah-first cold start where a 2-means clustering would
// collapse.
...
// SOM character match, and hard-classify it (dit/dah) to maintain the dit and
// dah length references and the fallback string. The hard call is only for
// reference tracking + fallback; the actual character decision is the SOM's
// (decodeChar).
void decodeMark(Channel& ch, int durHops, double hopMs) {
  ...
  if (!dah) {
    ...
    if (ch.markUnit <= 0.0)
      ch.markUnit = d;  // first-ever mark seeds even if a dah
    // Track the dah length toward the dah cluster (EMA; the SOM's "3-unit"
    // ref).
  ...
  const double unit =
      (ch.markUnit + (ch.gapUnit > 0 ? ch.gapUnit : ch.markUnit)) / 2.0;
  ch.wpm = std::clamp(
      static_cast<int>(std::lround(1200.0 / std::max(1.0, unit * hopMs))), 5,
      80);
...
// space — once each. Doing this *during* the gap (not when the next mark
// starts) means the last character of a transmission is emitted instead of
// being lost.
void growGap(Channel& ch, int gapHops, const CallSet* db) {
  ...
  if (!ch.wordDone && units >= 6.0 && !ch.text.empty() &&
      ch.text.back() != ' ') {
    appendChar(ch, ' ', /*confident=*/true,
               db);  // a separator, never part of a token
    ch.wordDone = true;
    ch.dirty = true;
  }
}
  ...
  while (std::getline(in, line)) {
    ...
    while (i < line.size()) {
      ...
      if (j > i) {
        ...
        if (tok.size() >= 3 && tok.size() <= 12) set->insert(tok);
      }
      i = (j > i) ? j : i + 1;
    }
  }
...
void CwSkimmer::pushPcm(const std::int16_t* samples, int frames, int channels,
                        int sampleRate) {
  if (!running_.load() || frames <= 0 || channels <= 0) return;
  std::lock_guard<std::mutex> lk(mu_);
  if (sampleRate != rate_)
  ...
  if (queue_.size() > cap) queue_.erase(queue_.begin(), queue_.end() - cap / 2);
...
void CwSkimmer::worker(SkimmerConfig cfg) {
  ...
  // not the FFT. hop is fftSize/8 (~5 ms steps) for responsive envelope
  // sampling.
  ...
  const int removeAfterHops =
      static_cast<int>(30000.0 / hopMs);  // drop after 30 s idle
  ...
  const int spawnPersist =
      3;  // frames a peak must persist before a channel spawns
  // Channels must be at least the per-channel receiver's bandwidth apart, or
  // two of them sit inside one carrier's passband and decode it identically
  // (the "same signal on several frequencies" duplicate). ~2x the 70 Hz
  // low-pass.
  ...
  while (running_.load()) {
    ...
    if (decim == 1) {
      ...
      for (; i + n <= aaHist.size(); i += decim) {
        float acc = 0.0f;
        for (std::size_t j = 0; j < n; ++j) acc += aaTaps[j] * aaHist[i + j];
        buf.push_back(acc);
      }
    ...
    while (buf.size() - pos >= fftSize) {
      ...
      const float spawnTh =
          floor * 7.0f * gateLin;  // peak strength to start a channel
      const double minSnr =
          minSnrDb_.load(std::memory_order_relaxed);  // per-channel gate
      ...
      auto nearChannel = [&](int k) {
        for (auto& [id, ch] : channels)
          if (std::abs(ch.bin - k) < minSepBins) return true;
        return false;
      };
      ...
      if (channels.size() < kMaxChannels) {
        for (int k = kMin + 1; k < kMax; ++k) {
          ...
          for (int j = -(minSepBins - 1); j <= minSepBins - 1; ++j) {
            const int n = k + j;
            if (n >= kMin && n <= kMax && mag[n] > mag[k]) {
              peak = false;
              break;
            }
          }
          if (!peak) continue;
          if (nearChannel(k) || isHarmonic(k)) continue;
      ...
      for (auto it = channels.begin(); it != channels.end();) {
        ...
        for (std::size_t i = 0; i < hop; ++i) {
          ...
          ch.loRe = nlo;
          ch.f1Re += lpA * (bRe - ch.f1Re);
          ch.f1Im += lpA * (bIm - ch.f1Im);
          ch.f2Re += lpA * (ch.f1Re - ch.f2Re);
          ch.f2Im += lpA * (ch.f1Im - ch.f2Im);
        ...
        if (ch.noisePow <= 0.0)
          ch.noisePow = P;
        else if (P < ch.noisePow)
          ch.noisePow += 0.1 * (P - ch.noisePow);
        else
          ch.noisePow += 0.002 * (P - ch.noisePow);
        ...
        if (ch.snrFloor <= 0.0)
          ch.snrFloor = P;
        else if (P < ch.snrFloor)
          ch.snrFloor += 0.5 * (P - ch.snrFloor);
        else
          ch.snrFloor += 0.0003 * (P - ch.snrFloor);
        // Track the key-down level (fast up, slow down) as a strength proxy
        // (the per-channel SNR numerator, and the ghost-dedup tiebreak).
        if (P > ch.peakPow)
          ch.peakPow = P;
        else
          ch.peakPow += 0.001 * (P - ch.peakPow);
        const double zdb =
            10.0 * std::log10((P + 1e-12) / (ch.noisePow + 1e-12));
        ...
        const double chSnr =
            10.0 * std::log10((ch.peakPow + 1e-12) / (ch.snrFloor + 1e-12));
        if (chSnr < minSnr) pOn = 0.0;
        ...
        const double onFromOn = ch.dOn + kLogStay,
                     onFromOff = ch.dOff + kLogFlip;
        const bool onPrevOn = onFromOn >= onFromOff;
        const double offFromOff = ch.dOff + kLogStay,
                     offFromOn = ch.dOn + kLogFlip;
        ...
        if (ch.vframes >= kViterbiLag) {
          ...
          if (!ch.started) {
            ...
            ++ch.runHops;
            if (!ch.curState)
              growGap(ch, ch.runHops, dbp);  // close char/word as silence grows
          } else if (st) {                   // gap -> mark: the gap just ended
            const double gu = ch.gapUnit > 0.0 ? ch.gapUnit : ch.markUnit;
            if (ch.markUnit > 0.0 && ch.runHops < 1.8 * gu)
              unitTrack(ch.gapUnit,
                        ch.runHops);  // an element gap defines the gap unit
            ...
            if (ch.markUnit > 0.0)
              ch.gapUnit =
                  std::clamp(ch.gapUnit, 0.7 * ch.markUnit, 1.3 * ch.markUnit);
            ch.curState = true;
            ch.runHops = 1;
          ...
          if (!ch.curState && ch.symbol.empty())
            ++ch.idleHops;
          else
            ch.idleHops = 0;
          ch.keyHist =
              (ch.keyHist << 1) | static_cast<unsigned __int128>(st ? 1 : 0);
        }
        ...
        if (ch.idleHops > removeAfterHops ||
            (knownOnly && ch.shown && !ch.callKnown &&
             ch.idleHops > newStationHops)) {
          if (ch.shown) postRemoved(it->first);
      ...
      for (auto a = channels.begin(); a != channels.end(); ++a)
        for (auto b = std::next(a); b != channels.end(); ++b) {
          ...
          if (!dup) {
            ...
            if (uni >= 24 && inter * 100 >= 65 * uni) dup = true;
          }
          if (dup) {
            // Keep the stronger signal (a ghost is the attenuated copy);
            // fall back to the cleaner/longer decode if strengths tie.
            bool keepA;
            if (A.peakPow > B.peakPow * 1.15)
              keepA = true;
            else if (B.peakPow > A.peakPow * 1.15)
              keepA = false;
            else
              keepA = A.richChars != B.richChars
      ...
      for (int id : toDrop) {
        ...
        if (it->second.shown) postRemoved(id);
        channels.erase(it);
      }
      ...
      //   - a constant baseline offset (bwOffsetDb_), applied first — a
      //   positive
      ...
      float bwOffDb =
          static_cast<float>(bwOffsetDb_.load(std::memory_order_relaxed));
      {
        ...
        if (bwDb > 0 && bwRef > 0 && bwHz > 0 && bwHz < bwRef)
          bwOffDb += std::clamp(static_cast<float>(bwDb) *
                                    static_cast<float>(std::log2(
                                        static_cast<double>(bwRef) / bwHz)),
                                0.0f, 45.0f);
      }
      ...
      for (int c = 0; c < kDisplayCols; ++c) {
        ...
        const int k1 =
            std::max(k0 + 1, kMin + (kMax - kMin) * (c + 1) / kDisplayCols);
        float m = 0.0f;
        for (int k = k0; k < k1 && k <= kMax; ++k) m = std::max(m, mag[k]);
        const float db = 20.0f * std::log10(m + 1e-6f);
        const float fl = 20.0f * std::log10(floor + 1e-6f);
        rowMax[c] = std::max(
            rowMax[c], std::clamp((db - fl - bwOffDb) / 45.0f, 0.0f, 1.0f));
      }
...
void CwSkimmer::postWaterfall(std::vector<float> mags, double minHz,
                              double maxHz) {
  ui_.post([this, w = std::weak_ptr<bool>(alive_), mags = std::move(mags),
            minHz, maxHz]() {
    if (!w.expired() && onWaterfall) onWaterfall(mags, minHz, maxHz);
  });
}

void CwSkimmer::postChannel(int id, double hz, int wpm, std::string text,
                            std::string call) {
