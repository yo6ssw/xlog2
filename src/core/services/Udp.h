...
// Listens on a UDP port and reports any decoded QSOs through a callback. This
// is xlog's network-logging idea: other programs (WSJT-X, fldigi, JTAlert, …)
// send logged contacts and they are added to the logbook automatically.
//
// A worker thread blocks in recv(); decoded QSOs are marshalled to the UI
// thread via the injected dispatcher, so the callback always fires on the UI
// thread. A self-pipe unblocks the worker promptly on stop().
class UdpListener {
 public:
  using Callback =
