...
// thread polls them every ~250 us and ticks the element generator, emitting
// edges as UDP datagrams (each carrying recent edges as loss-recovery history)
// and a keepalive while idle so cwsd keeps the session anchored. Edge
// timestamps come from the element *schedule*, not the poll clock, so the
// keying stays exact — the poll interval bounds only how fast we react to a
// fresh paddle close.
//
// Local sidetone gives the operator instant feel: a native PipeWire stream
// renders a click-free (ramped-envelope) sine, gated by the same key-down/up
...
// does not. The sidetone is independent: if its sink is missing, keying still
// streams.
//
// Autospace (on by default): when a new character's first element is keyed
// within 3 dits of the previous element ending, its start is held to the 3-dit
// boundary so quickly-tapped letters get a clean inter-character space instead
// of running together. It applies only across the idle gap between characters,
// never to a continuing iambic squeeze.
//
// NOTE (scaffold): iambic memory gives iambic-A behaviour; full iambic-B is a
// TODO.
//
// Ultimatic ("last-pressed memory", `ultimatic` below): an alternative to
// iambic alternation. While both paddles are held, the keyer sends the element
// of whichever paddle was *pressed most recently* and repeats it until the
// other is pressed again — last press wins. A press of either paddle during an
// element is remembered so a quick tap is never dropped. When set, it overrides
// the iambic A/B behaviour.
struct RemotePaddleConfig {
  ...
  // Local-only mode: run the iambic element generator and the local sidetone
  // but skip all cwsd networking (no socket, no edge/keepalive datagrams). Used
  // by the standalone xlog2-paddle practice tool — sidetone feel without remote
  // keying.
  ...
  // Sidetone PipeWire node quantum, in frames @48 kHz: this is the dominant
  // feel latency (512 ≈ 10.7 ms). A smaller value tightens the feel but pulls
  // the whole PipeWire graph to a smaller quantum, which can xrun other streams
  // (e.g. the rig-audio stream) — so the main app keeps 512. The standalone
  // xlog2-paddle tool has no other streams and lowers it (≈128 ≈ 2.7 ms).
...
class RemotePaddleKeyer {
 public:
  std::function<void(const std::string&)>
      onStatus;  // connection state / errors (UI thread)
 ...
 private:
  ...
  // Liveness token for posted closures (mirrors AudioStreamClient): recreated
