...
const std::vector<BandRange> kBands = {
    {"2190m", 0.1357, 0.1378}, {"630m", 0.472, 0.479}, {"160m", 1.8, 2.0},
    {"80m", 3.5, 4.0},         {"60m", 5.06, 5.45},    {"40m", 7.0, 7.3},
...
const std::vector<std::string>& names() {
  static const std::vector<std::string> n = [] {
    ...
    for (const auto& b : kBands) v.emplace_back(b.name);
    return v;
  }();
  return n;
}
...
const std::vector<std::string>& modes() {
  static const std::vector<std::string> m = {
      "SSB",   "USB", "LSB", "CW",   "AM",  "FM",   "RTTY",
      "PSK31", "FT8", "FT4", "JT65", "JS8", "MFSK", "DIGITAL",
  };
  return m;
}
...
std::string forFrequencyMHz(double mhz) {
  for (const auto& b : kBands) {
    if (mhz >= b.low && mhz <= b.high) return b.name;
  }
  return {};
}
