...
void addToken(std::string token, const Info& entityDefaults) {
  ...
  for (; i < token.size();) {
    ...
    if (open == '(')
      info.cqZone = std::atoi(inner.c_str());
    else if (open == '[')
      info.ituZone = std::atoi(inner.c_str());
    else if (open == '{')
      info.continent = inner;
...
// Letters trailing the last digit ("W1AW"->2, "VE3"->0, "DL"->0 since no
// digit).
int suffixLen(const std::string& t) {
  int lastDigit = -1;
  for (int i = 0; i < static_cast<int>(t.size()); ++i)
    if (std::isdigit(static_cast<unsigned char>(t[i]))) lastDigit = i;
  if (lastDigit < 0) return 0;  // a bare prefix such as "DL" or "F"
  int n = 0;
  for (int i = lastDigit + 1; i < static_cast<int>(t.size()); ++i)
    if (std::isalpha(static_cast<unsigned char>(t[i]))) ++n;
  return n;
}
...
bool isModifier(const std::string& t) {
  ...
  if (allDigits) return true;  // e.g. "/7" US call-area change — same entity
...
std::string resolvePortable(const std::string& call) {
  ...
  for (const auto& t : tokens) {
    ...
    if (isModifier(t)) continue;
    kept.push_back(t);
  }
  if (kept.empty()) return tokens[0];
  ...
  for (const auto& t : kept) {
    ...
    if (s < bs || (s == bs && t.size() < best->size())) best = &t;
  }
  return *best;
}
...
void ensureLoaded() {
  if (g_loaded || g_attempted) return;
  g_attempted = true;
...
const Info* lookup(const std::string& call) {
  ...
  if (auto it = g_exact.find(c); it != g_exact.end()) return &it->second;
  ...
  for (size_t len = key.size(); len > 0; --len) {
    ...
    if (it != g_prefixes.end()) return &it->second;
  }
  return nullptr;
}
