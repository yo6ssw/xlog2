...
constexpr std::array<const char*, 24> kColumns = {
    "date",    "time_on",   "time_off",       "call",      "band",
    "mode",    "freq",      "rst_sent",       "rst_rcvd",  "name",
    "qth",     "locator",   "power",          "qsl_sent",  "qsl_rcvd",
    "comment", "lotw_sent", "lotw_sent_date", "lotw_rcvd", "lotw_rcvd_date",
    "country", "cq_zone",   "itu_zone",       "continent",
};

std::array<std::string, kColumns.size()> fieldsOf(const Qso& q) {
  return {q.date,      q.time_on,        q.time_off,  q.call,
          q.band,      q.mode,           q.freq,      q.rst_sent,
          q.rst_rcvd,  q.name,           q.qth,       q.locator,
          q.power,     q.qsl_sent,       q.qsl_rcvd,  q.comment,
...
std::string contentKey(const Qso& q) {
  const char sep = '\x1f';
  return q.call + sep + q.band + sep + q.mode + sep + q.date + sep + q.time_on +
         sep + q.time_off + sep + q.freq + sep + q.rst_sent + sep + q.rst_rcvd +
         sep + q.comment;
}
...
LogBook::LogBook() { newInMemory(); }

LogBook::~LogBook() { close(); }
  ...
  // logbook by peers.
  {
    ...
    if (!sid.empty()) {
      ...
      if (sqlite3_prepare_v2(
              db,
              "INSERT OR REPLACE INTO meta (key, value) VALUES ('sync_id', ?);",
...
bool LogBook::remove(long id) { return removeTracked(id).ok; }

RemoveResult LogBook::removeTracked(long id) {
  RemoveResult r;
  if (!db_ || id == 0) return r;
...
std::string LogBook::exportAdif() const { return adif::write(cache_); }

std::optional<Qso> LogBook::findDuplicate(const Qso& q, long excludeId,
                                          DupeRule rule) const {
  if (q.call.empty()) return std::nullopt;
  ...
  for (auto it = cache_.rbegin(); it != cache_.rend(); ++it) {
    ...
    if (!iequals(e.call, q.call)) continue;
    switch (rule) {
      ...
      case DupeRule::CallBandMode:
        if (e.band == q.band && iequals(e.mode, q.mode)) return e;
        break;
      case DupeRule::Call:
        return e;
    }
...
std::vector<Qso> LogBook::qsosNotLotwSent() const {
  std::vector<Qso> out;
  for (const auto& q : cache_)
    if (q.lotw_sent != "Y") out.push_back(q);
  return out;
}

void LogBook::markLotwSent(const std::vector<long>& ids,
  ...
  for (const auto& u : updates) update(u);
}
...
int LogBook::applyLotwConfirmations(const std::vector<Qso>& confirmed) {
  ...
  for (const auto& c : confirmed) {
    for (const auto& e : cache_) {
      if (e.lotw_rcvd == "Y" || used.count(e.id)) continue;
      ...
      if (et >= 0 && ct >= 0 && std::abs(et - ct) > kTimeToleranceMin) continue;
...
void LogBook::migrateUuids() {
  ...
  if (sqlite3_prepare_v2(db_,
                         "UPDATE qsos SET uuid=?, updated_at=? WHERE id=?;", -1,
...
std::string LogBook::metaGet(const std::string& key) const {
  ...
  bindText(stmt, 1, key);
  std::string out;
  if (sqlite3_step(stmt) == SQLITE_ROW) out = columnText(stmt, 0);
  sqlite3_finalize(stmt);
  return out;
}
...
void LogBook::setSyncId(const std::string& id) { metaSet("sync_id", id); }

SyncManifest LogBook::syncManifest() const {
  SyncManifest m;
  m.records.reserve(cache_.size());
  for (const auto& q : cache_)
    if (!q.uuid.empty()) m.records.push_back({q.uuid, q.updated_at});

  if (db_) {
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, "SELECT uuid, deleted_at FROM tombstones;", -1,
                           &stmt, nullptr) == SQLITE_OK) {
      while (sqlite3_step(stmt) == SQLITE_ROW)
...
std::vector<Qso> LogBook::recordsByUuids(
  ...
  std::vector<Qso> out;
  for (const auto& q : cache_)
    if (!q.uuid.empty() && want.count(q.uuid)) out.push_back(q);
  return out;
}
...
MergeResult LogBook::applyRemote(const std::vector<Qso>& records,
  ...
  std::unordered_map<std::string, std::pair<long, std::string>>
      live;  // uuid -> (id, updated_at)
  for (const auto& q : cache_)
    if (!q.uuid.empty()) live[q.uuid] = {q.id, q.updated_at};
  ...
  for (const auto& r : records) {
    ...
    if (l == live.end()) {
      ...
      const bool remoteWins = r.updated_at > localStamp ||
