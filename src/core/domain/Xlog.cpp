...
// Trims leading/trailing ASCII whitespace (incl. a trailing CR from CRLF
// files).
...
std::string dateFromXlog(const std::string& v) {
  ...
  if (day.empty() || mon.empty() || year.size() != 4) return v;
...
void setField(Qso& q, const std::string& field, const std::string& value) {
  ...
  if (field == "date")
    q.date = dateFromXlog(value);
  else if (field == "gmt" || field == "utc")
    q.time_on = timeFromXlog(value);
  else if (field == "gmtend" || field == "utcend")
    q.time_off = timeFromXlog(value);
  else if (field == "call")
...
std::vector<Qso> parse(const std::string& text) {
  ...
  while (nextLine(line)) {
    ...
    for (size_t c = 0; c < cols.size(); ++c) {
      ...
      const size_t end =
          (c + 1 < cols.size()) ? cols[c + 1].start : line.size();
      setField(q, cols[c].field, trim(line.substr(start, end - start)));
    }
    if (!q.call.empty()) out.push_back(q);
  }
