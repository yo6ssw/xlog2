...
std::string serialiseTrustedPeers(
    const std::vector<Settings::SyncTrustedPeer>& v) {
  std::string out;
  for (const auto& tp : v) {
...
Settings Settings::load(const IniFile& ini) {
  ...
  s.lotwLastDownload =
      ini.getString("lotw", "last_download", s.lotwLastDownload);
  ...
  for (int i = 0; i < 9; ++i)
    s.keyerMessages[i] = ini.getString(
        "keyer", "message" + std::to_string(i + 1), s.keyerMessages[i]);
  ...
  s.paddleSidetoneDevice =
      ini.getString("paddle", "sidetone_device", s.paddleSidetoneDevice);
  ...
  s.skimmerBwNormRefHz =
      ini.getInt("skimmer", "bw_norm_ref_hz", s.skimmerBwNormRefHz);
  s.skimmerBwOffsetDb =
      ini.getInt("skimmer", "bw_offset_db", s.skimmerBwOffsetDb);
  ...
  s.syncRequireIdentity =
      ini.getBool("sync", "require_identity", s.syncRequireIdentity);
  s.syncNodeName = ini.getString("sync", "node_name", s.syncNodeName);
  s.syncTrustedPeers =
      parseTrustedPeers(ini.getString("sync", "trusted_peers", ""));
...
void Settings::store(IniFile& ini) const {
  ...
  if (rigVisible && rigPanelPos > 0) ini.setInt("rig", "position", rigPanelPos);
  ...
  ini.setString("sync", "trusted_peers",
                serialiseTrustedPeers(syncTrustedPeers));
