...
// Parse a 2/4/6/8-character Maidenhead locator (case-insensitive) and return
// the centre of the addressed square. Returns nullopt for structurally invalid
// input (so a half-typed field simply draws nothing).
std::optional<LatLon> maidenheadToLatLon(std::string grid);

// Maidenhead grid (6 characters) for a coordinate — handy for
// tests/round-trips.
...
// Points along the great circle from a to b (inclusive of both ends), sampled
// at `segments`+1 positions via spherical interpolation — a smooth curve when
// drawn in the equirectangular projection.
