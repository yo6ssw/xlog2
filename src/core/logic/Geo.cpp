...
std::optional<LatLon> maidenheadToLatLon(std::string g) {
  ...
  if (n != 2 && n != 4 && n != 6 && n != 8) return std::nullopt;
  ...
  if (!isField(g[0]) || !isField(g[1])) return std::nullopt;
  ...
  if (n >= 8) {
    ...
    lonCell = (2.0 / 24.0) / 10.0;
    latCell = (1.0 / 24.0) / 10.0;
    lon += (g[6] - '0') * lonCell;
    lat += (g[7] - '0') * latCell;
  }
...
std::vector<LatLon> greatCircle(LatLon a, LatLon b, int segments) {
  ...
  for (int i = 0; i <= segments; ++i) {
    ...
    out.push_back(
        {deg(std::atan2(z, std::sqrt(x * x + y * y))), deg(std::atan2(y, x))});
  }
  return out;
}
...
std::vector<std::vector<LatLon>> loadCoastline(const std::string& path) {
  ...
  if (path.empty()) return polylines;
  std::ifstream in(path);
  if (!in) return polylines;
  ...
  while (skipComments(), in >> n) {
    ...
    for (long i = 0; i < n; ++i) {
      double lon = 0, lat = 0;
      if (!(in >> lon >> lat)) return polylines;  // truncated; keep what parsed
