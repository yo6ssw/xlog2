...
// rtkit refuses a thread that has no finite RLIMIT_RTTIME (so a runaway RT
// thread can't lock the machine), so we set that from RTTimeUSecMax before
// asking.
namespace platform {
namespace {
...
// Read an rtkit integer property; the value is wrapped in a variant that may
// hold an INT32 (MaxRealtimePriority, MinNiceLevel) or INT64 (RTTimeUSecMax).
...
bool callMakeRealtime(DBusConnection* conn, pid_t tid, std::uint32_t prio) {
  DBusMessage* msg = dbus_message_new_method_call(kService, kPath, kIface,
                                                  "MakeThreadRealtime");
  if (msg == nullptr) return false;
  dbus_uint64_t thread = static_cast<dbus_uint64_t>(tid);
  dbus_uint32_t p = prio;
  dbus_message_append_args(msg, DBUS_TYPE_UINT64, &thread, DBUS_TYPE_UINT32, &p,
                           DBUS_TYPE_INVALID);
...
bool makeThreadRealtime(int priority) {
  DBusError err;
  dbus_error_init(&err);
  // A private connection so we own its lifetime and don't disturb any shared
  // bus.
  ...
  // rtkit requires a finite RLIMIT_RTTIME on the thread; mirror its advertised
  // cap.
  ...
  if (getIntProperty(conn, "MaxRealtimePriority", maxPrio) &&
      priority > maxPrio)
    priority = static_cast<int>(maxPrio);
  if (priority < 1) priority = 1;

  const bool ok =
      callMakeRealtime(conn, threadId(), static_cast<std::uint32_t>(priority));
