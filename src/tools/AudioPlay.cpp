...
// FEC/PLC loss recovery, drift-compensated jitter buffer). Run it, Ctrl-C to quit.
...
// AudioStreamClient marshals worker-thread callbacks through an IUiDispatcher. We
// have no UI thread, so just run each closure inline on the calling thread; the
// callbacks here only print, which is fine from the worker thread.
...
int main() {
    ...
    if (!ini.loadFromFile(path))
        std::cerr << "xlog2-audioplay: could not read " << path << " (using defaults)\n";
    ...
    client.onStatus = [](const std::string& msg) { std::cout << msg << std::endl; };
