...
// generator and the click-free local sidetone run exactly as in the full app, but
// nothing is keyed over the network — there is no cwsd connection. The window just
// exposes the paddle settings (speed, iambic type, autospace) plus the sidetone
// controls, so you can practise feel and timing with only audio coming out.
//
// Paddle input: a real USB HID paddle (the companion firmware) if present, and the
...
// Settings persist in their own ini, separate from the main app's layout.ini, so
// the two never clobber each other. Mirrors xlog2's $XDG_CONFIG_HOME/xlog2 layout.
...
public:
    PaddleWindow() : keyer_(dispatcher_), hid_(dispatcher_) {
        ...
        loadSettings();  // populate cfg_ before the widgets read their initial values
        ...
        auto setWpmLabel = [this](int v) { wpmLabel_->setText(QString::number(v) + " wpm"); };
        ...
        auto setToneHzLabel = [this](int v) { toneHzLabel_->setText(QString::number(v) + " Hz"); };
        ...
        auto setLevelLabel = [this](int v) { levelLabel_->setText(QString::number(v) + " %"); };
        ...
        keyer_.onStatus = [this](const std::string& s) { status_->setText(QString::fromStdString(s)); };
        hid_.onStatus   = [this](const std::string& s) { status_->setText(QString::fromStdString(s)); };
        // HID contacts fire on the worker thread; setDit/setDah are lock-free atomics.
        ...
        auto restart = [this] { if (keyer_.isActive()) start(); };
        ...
        connect(iambic_, qOverload<int>(&QComboBox::currentIndexChanged), this, restart);
        connect(autospace_, &QCheckBox::toggled, this, restart);

        qApp->installEventFilter(this);  // for the `[` / `]` paddle simulation

        startStop_->setChecked(true);    // autostart the keyer on launch (fires onToggle)
    }
...
protected:
    ...
    // App-wide filter: while keying, `[`/`]` drive dit/dah and are consumed so the
    // brackets aren't typed; otherwise events pass through untouched.
...
private:
    ...
    void start() {
        ...
        cfg_.sidetone   = true;
        // No other PipeWire streams to protect here, so use a small quantum (~2.7 ms)
        // for the tightest sidetone feel — far below the main app's graph-safe 512.
    ...
    // Persist the current control values. Loads first so any unrelated keys survive.
    void saveSettings() {
        IniFile ini;
