...
//
// It joins the same peer-to-peer sync mesh the GUIs use and keeps a full
// replica of the default logbook (every add/edit/delete on any node gossips to
// it), and it answers the mesh's distributed QRZ-cache queries. Because that
// QRZ protocol is pull-only (a peer's rich cache is never pushed), the daemon
// also *actively enriches*: it looks up newly-synced callsigns on qrz.com using
// the configured credentials, so its qrz-cache.sqlite grows into a durable
// cache other nodes can resolve against.
//
  ...
  void drain(std::chrono::duration<Rep, Period> timeout) {
    std::deque<std::function<void()>> batch;
    {
      ...
      cv_.wait_for(lk, timeout,
                   [&] { return !q_.empty() || !g_running.load(); });
      batch.swap(q_);
    }
...
void usage(const char* argv0) {
  std::cout << "Usage: " << argv0 << " [--config PATH] [--data-dir PATH]\n\n"
            << "Headless xlog2 backup peer: replicates the synced logbook and\n"
            << "enriches the shared qrz.com cache.\n\n"
            << "  --config PATH    layout.ini to read (default: "
            << defaultConfigPath() << ")\n"
            << "  --data-dir PATH  where to keep this peer's default.xlog,\n"
            << "                   qrz-cache.sqlite and node_id (default: "
            << defaultDataDir() << ")\n"
            << "  --port N         fixed TCP listen port so other nodes can "
               "dial this peer\n"
            << "                   as a static [sync] peer_host (default: 0 = "
               "ephemeral,\n"
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--config")
      ...
      std::cerr << "xlog2-syncd: unknown option '" << a << "'\n";
      usage(argv[0]);
      return 2;
    }
  }
  ...
  if (!ini.loadFromFile(configPath))
    std::cerr << "xlog2-syncd: could not read " << configPath
              << " (using defaults)\n";
  ...
  sync.onMessage = [&](const LogbookSync::PeerKey& p,
  ...
  std::set<std::string>
      attempted;  // per-session: don't re-fetch no-record calls

  auto pumpEnrich = [&]() {
    if (!enrichEnabled || qrz.isBusy() || enrichQueue.empty()) return;
    std::string call = enrichQueue.front();
    enrichQueue.pop_front();
    qrz.lookup(s.qrzUser, s.qrzPassword,
               call);  // result lands in qrz-cache.sqlite
  };
  ...
  auto sweepEnrich = [&]() {
    ...
    for (const Qso& q : page.logbook().qsos()) {
      if (q.call.empty() || attempted.count(q.call)) continue;
  ...
  coord.setTrust(/*enforce=*/false,
                 {});  // backup peer: trust all same-secret nodes

  std::cout << "xlog2-syncd: node " << sync.localId() << "  group " << cfg.group
  ...
  auto lastSweep =
      clock::now() - std::chrono::seconds(60);  // sweep immediately
