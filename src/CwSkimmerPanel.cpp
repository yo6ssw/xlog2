...
uint32_t heat(float v) {
  ...
  constexpr int N = 8;
  static const float pos[N] = {0.00f, 0.35f, 0.55f, 0.68f,
                               0.78f, 0.86f, 0.93f, 1.00f};
  ...
  const uint32_t r =
      static_cast<uint32_t>(col[i][0] + t * (col[i + 1][0] - col[i][0]));
  const uint32_t g =
      static_cast<uint32_t>(col[i][1] + t * (col[i + 1][1] - col[i][1]));
  const uint32_t b =
      static_cast<uint32_t>(col[i][2] + t * (col[i + 1][2] - col[i][2]));
  return (r << 16) | (g << 8) | b;
}
...
// genuinely-new characters. Capped so a long-lived channel can't grow
// unbounded.
void appendWindow(std::string& full, const std::string& window) {
  ...
  if (full.size() > kCap) full.erase(0, full.size() - kCap);
}
...
CwSkimmerPanel::CwSkimmerPanel() : Gtk::Box(Gtk::Orientation::VERTICAL) {
  ...
  waterfall_.set_draw_func(
      sigc::mem_fun(*this, &CwSkimmerPanel::onDrawWaterfall));
  append(waterfall_);
  ...
  Glib::signal_timeout().connect(
      sigc::mem_fun(*this, &CwSkimmerPanel::drainWaterfall), 16);
  ...
  gateScale_->signal_value_changed().connect([this]() {
    ...
    if (!updatingGate_) signalGate_.emit(db);
  });
  ...
  snrScale_->signal_value_changed().connect([this]() {
    ...
    if (!updatingSnr_) signalMinSnr_.emit(db);
  });
  ...
  // --
  knownOnly_ =
      Gtk::make_managed<Gtk::CheckButton>("Show only calls in database");
  knownOnly_->set_sensitive(false);  // until a DB is loaded (see setCallDbInfo)
  knownOnly_->signal_toggled().connect([this]() {
    if (!updatingKnown_) signalKnownOnly_.emit(knownOnly_->get_active());
  });
  append(*knownOnly_);
  ...
  columnView_.append_column(makeColumn(
      "Freq", [](const SkimmerItem& i) { return i.freq.get_value(); }));
  columnView_.append_column(makeColumn(
      "WPM", [](const SkimmerItem& i) { return i.wpm.get_value(); }));
  columnView_.append_column(makeColumn(
      "Text", [](const SkimmerItem& i) { return i.text.get_value(); }, true));
  columnView_.append_column(makeColumn(
      "Call", [](const SkimmerItem& i) { return i.call.get_value(); }));

  // Double-click (or Enter) on a row pops up the channel's complete decode.
  columnView_.signal_activate().connect([this](guint pos) {
    if (auto item = store_->get_item(pos)) showChannelText(item->id);
  });
...
Glib::RefPtr<Gtk::ColumnViewColumn> CwSkimmerPanel::makeColumn(
    ...
    std::function<Glib::ustring(const SkimmerItem&)> getter, bool expand) {
  ...
  auto conns = std::make_shared<
      std::map<Gtk::ListItem*, std::vector<sigc::connection>>>();
  factory->signal_bind().connect([getter, conns](
  ...
  factory->signal_unbind().connect(
      [conns](const Glib::RefPtr<Gtk::ListItem>& li) {
...
void CwSkimmerPanel::showChannelText(int id) {
  ...
  win->signal_hide().connect([this, id, win]() {
    ...
    if (auto it = textWindows_.find(id);
        it != textWindows_.end() && it->second == win) {
...
void CwSkimmerPanel::addWaterfall(const std::vector<float>& mags, double minHz,
  ...
  while (static_cast<int>(pending_.size()) > kHistory) pending_.pop_front();
}
...
void CwSkimmerPanel::scrollOneRow(const std::vector<float>& mags) {
  ...
  std::memmove(
      pixels_.data() + cols_, pixels_.data(),
      static_cast<std::size_t>(cols_) * (kHistory - 1) * sizeof(uint32_t));
  for (int x = 0; x < cols_; ++x) pixels_[x] = heat(mags[x]);
}

bool CwSkimmerPanel::drainWaterfall() {
  if (pending_.empty()) return true;  // keep the timer running
  // One row per tick normally; advance a couple when a burst has piled up so
  // the backlog drains without a visible multi-pixel jump.
...
void CwSkimmerPanel::onDrawWaterfall(const Cairo::RefPtr<Cairo::Context>& cr,
                                     int w, int h) {
  cr->set_source_rgb(0, 0, 0);
  cr->paint();
  if (cols_ > 0 && !pixels_.empty()) {
    const int stride = Cairo::ImageSurface::format_stride_for_width(
        Cairo::Surface::Format::RGB24, cols_);
    ...
    cr->save();
    cr->scale(static_cast<double>(w) / cols_,
              static_cast<double>(h) / kHistory);
    cr->set_source(surf, 0, 0);
    cr->paint();
    cr->restore();
  }
  if (maxHz_ <= minHz_) return;
  const double span = maxHz_ - minHz_;

  // Frequency grid + axis labels every 500 Hz.
  cr->set_font_size(10);
  for (int hz = static_cast<int>(std::ceil(minHz_ / 500) * 500); hz < maxHz_;
       hz += 500) {
  ...
  for (const auto& [hz, call] : labels_) {
    if (hz < minHz_ || hz > maxHz_) continue;
    const double x = (hz - minHz_) / span * w;
...
void CwSkimmerPanel::updateChannel(int id, double hz, int wpm,
...
void CwSkimmerPanel::setCallDbInfo(bool loaded, std::size_t count) {
  ...
  knownOnly_->set_label(loaded ? "Show only calls in database (" +
                                     std::to_string(count) + ")"
