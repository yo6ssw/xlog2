...
namespace {
// Parse an int entry, falling back to `def` on garbage (mirrors the old
// dialogs).
int toInt(const Gtk::Entry* e, int def) {
  try {
    return std::stoi(e->get_text().raw());
...
Gtk::Grid& SettingsDialog::addPage(Gtk::Stack& stack, const char* id,
                                   const char* title) {
...
SettingsDialog::SettingsDialog(const Settings& s,
  ...
  // --- LoTW ---
  {
    ...
    lotwPass_ = entry(s.lotwPassword);
    lotwPass_->set_visibility(false);
  ...
  // --- QRZ ---
  {
    ...
    qrzPass_ = entry(s.qrzPassword);
    qrzPass_->set_visibility(false);
  ...
  // --- CW Keyer ---
  {
    ...
    for (int i = 0; i < 9; ++i) {
      ...
      field(g, ("F" + std::to_string(i + 1) + ":").c_str(), *keyerMsgs_[i],
            3 + i);
    }
    auto* hint = Gtk::make_managed<Gtk::Label>(
  ...
  // --- Paddle ---
  {
    ...
    paddleIambicB_ =
        Gtk::make_managed<Gtk::CheckButton>("Iambic B (default: iambic A)");
    paddleIambicB_->set_active(s.paddleIambicB);
    paddleAutospace_ = Gtk::make_managed<Gtk::CheckButton>(
        "Autospace (enforce inter-character spacing)");
    ...
    paddleMute_ =
        Gtk::make_managed<Gtk::CheckButton>("Mute rig audio while keying");
  ...
  // --- Skimmer ---
  {
    ...
    skKnownOnly_ = Gtk::make_managed<Gtk::CheckButton>(
        "Paranoid: only surface DB-confirmed calls");
  ...
  // --- Sync ---
  {
    ...
    syncEnabled_ = Gtk::make_managed<Gtk::CheckButton>(
        "Synchronise the default logbook with LAN peers");
    syncEnabled_->set_active(s.syncEnabled);
    syncSecret_ = entry(s.syncSecret);
    syncSecret_->set_visibility(false);
    ...
    auto* hint = Gtk::make_managed<Gtk::Label>(
        "Peers on the same LAN find each other automatically — no roles to "
        "set.\n"
        "Give every machine the same secret: it picks the mesh and (via "
        "libsodium)\n"
        "encrypts and authenticates every peer link, and each node gets a\n"
        "self-certifying identity. Choose who to sync with under Sync ▸ "
        "Trusted\n"
  ...
  ok->signal_clicked().connect([this]() {
    onApply_(collect());
    set_visible(false);
  });
}
