...
class DxClusterPanel : public Gtk::Box {
 public:
  ...
  sigc::signal<void(const DxSpot&)>& signalActivate() {
    return signalActivate_;
  }
  sigc::signal<void(const std::string&)>& signalCommand() {
    return signalCommand_;
  }
 ...
 private:
  ...
  Glib::RefPtr<Gtk::ColumnViewColumn> makeColumn(
      ...
      std::function<std::string(const BandMapItem&)> getter,
      bool expand = false);
