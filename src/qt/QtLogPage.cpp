...
const char* const kColIds[] = {"date",  "on",   "off",  "call",   "country",
                               "cont",  "band", "mode", "freq",   "rst_s",
                               "rst_r", "name", "qth",  "loc",    "pwr",
                               "qsl",   "lotw", "cqz",  "comment"};
constexpr int kColCount =
    static_cast<int>(sizeof(kColIds) / sizeof(kColIds[0]));

int logicalForId(const std::string& id) {
  for (int i = 0; i < kColCount; ++i)
    if (id == kColIds[i]) return i;
  return -1;
}
}  // namespace
...
QtLogPage::QtLogPage(QWidget* parent) : QWidget(parent), presenter_(*this) {
  ...
  presenter_.onStatus = [this](const std::string& s) {
    emit status(QString::fromStdString(s));
  };
  presenter_.onLookupCall = [this](const std::string& c) {
    emit lookupCall(QString::fromStdString(c));
  };
  presenter_.onSendCw = [this](const std::string& t) {
    emit sendCw(QString::fromStdString(t));
  };
  presenter_.onAbortCw = [this]() { emit abortCw(); };
  presenter_.onLocator = [this](const std::string& g) {
    emit locatorChanged(QString::fromStdString(g));
  };
...
static QLineEdit* addField(QGridLayout* grid, const QString& label, int row,
                           int col) {
...
void QtLogPage::buildUi() {
  ...
  connect(header, &QWidget::customContextMenuRequested, this,
          [this, header](const QPoint& pos) {
            ...
            for (int i = 0; i < kColCount; ++i) {
              ...
              connect(a, &QAction::toggled, this, [header, i](bool on) {
                header->setSectionHidden(i, !on);
              });
            }
  ...
  band_->addItem("");
  for (const auto& b : bands::names())
    band_->addItem(QString::fromStdString(b));
  mode_->addItem("");
  for (const auto& m : bands::modes())
    mode_->addItem(QString::fromStdString(m));
  ...
  for (int i = 0; i < 9; ++i) {
    ...
    connect(b, &QPushButton::clicked, this,
            [this, i]() { presenter_.onSendCwClicked(i); });
    cwButtons_[i] = b;
  ...
  connect(stopButton, &QPushButton::clicked, this,
          [this]() { presenter_.onAbortCwClicked(); });
  ...
  connect(addButton_, &QPushButton::clicked, this,
          [this]() { presenter_.onAddOrUpdate(); });
  connect(deleteButton_, &QPushButton::clicked, this,
          [this]() { presenter_.onDelete(); });
  connect(clearButton, &QPushButton::clicked, this,
          [this]() { presenter_.onClear(); });
  connect(nowButton, &QPushButton::clicked, this,
          [this]() { presenter_.onSetNow(); });
  connect(lookupButton, &QPushButton::clicked, this,
          [this]() { presenter_.onLookupCallClicked(); });
  connect(call_, &QLineEdit::returnPressed, this,
          [this]() { presenter_.onAddOrUpdate(); });
  connect(call_, &QLineEdit::textChanged, this, [this]() {
    if (!loadingForm_) presenter_.onCallChanged();
  });
  connect(freq_, &QLineEdit::textChanged, this, [this]() {
    if (!loadingForm_) presenter_.onFreqChanged();
  });
  connect(date_, &QLineEdit::textChanged, this, [this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  connect(band_, &QComboBox::currentIndexChanged, this, [this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  connect(mode_, &QComboBox::currentIndexChanged, this, [this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  connect(locator_, &QLineEdit::textChanged, this, [this]() {
      ...
      for (const auto& [title, target] : targets)
        connect(sub->addAction(QString::fromStdString(title)),
                &QAction::triggered, this, [this, id, target]() {
                  if (requestMove) requestMove(id, target);
                });
    }
  }
...
void QtLogPage::confirmDeleteRow(long id) {
  ...
  if (QMessageBox::question(this, "Delete QSO",
                            QString("Delete %1?\n\nThis permanently removes "
                                    "the QSO from the logbook.")
...
void QtLogPage::setCall(const std::string& s) {
  call_->setText(QString::fromStdString(s));
}
void QtLogPage::setFreq(const std::string& s) {
  freq_->setText(QString::fromStdString(s));
}
void QtLogPage::setBand(const std::string& s) {
  band_->setCurrentText(QString::fromStdString(s));
}
void QtLogPage::setMode(const std::string& s) {
  mode_->setCurrentText(QString::fromStdString(s));
}

void QtLogPage::setRows(const std::vector<Qso>& qsos) { model_->setRows(qsos); }

void QtLogPage::clearSelection() { table_->selectionModel()->clearSelection(); }
...
void QtLogPage::setCwButtons(const std::array<std::string, 9>& messages) {
  for (int i = 0; i < 9; ++i) {
    ...
    cwButtons_[i]->setToolTip(has ? QString::fromStdString(messages[i])
                                  : "(no message set)");
  }
}
...
void QtLogPage::applyColumnLayout(const IniFile& ini) {
  ...
  if (ini.hasKey("columns", "order")) {
    int target = 0;
    for (const auto& id :
         strutil::splitSemicolons(ini.getString("columns", "order"))) {
      const int logical = logicalForId(id);
      if (logical < 0) continue;
      const int cur = h->visualIndex(logical);
      if (cur >= 0 && cur != target) h->moveSection(cur, target);
      ++target;
    }
  }
}
...
void QtLogPage::storeColumnLayout(IniFile& ini) const {
  ...
  for (int v = 0; v < kColCount; ++v) {
    ...
    if (logical < 0 || logical >= kColCount) continue;
  ...
  for (int i = 0; i < kColCount; ++i) {
    ...
    if (w > 0) ini.setInt("width", kColIds[i], w);
