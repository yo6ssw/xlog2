...
void QtDxClusterPanel::applyMutable(int row, const Entry& e,
  ...
  for (const auto* p : sorted) {
    const auto mins =
        std::chrono::duration_cast<std::chrono::minutes>(now - p->second.time)
            .count();
    if (!tip.isEmpty()) tip += '\n';
    tip += QString("%1  (%2m ago)")
               .arg(QString::fromStdString(p->first))
               .arg(mins);
  }
  ...
  if (auto* count = model_->item(row, 3)) {
    ...
    if (count->text() != text) count->setText(text);
    if (count->toolTip() != tip) count->setToolTip(tip);
  }
