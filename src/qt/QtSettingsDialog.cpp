...
// Fall back to `def` when the field is left blank (host fields must not be
// empty).
...
QtSettingsDialog::QtSettingsDialog(const Settings& s, QWidget* parent)
  ...
  connect(list_, &QListWidget::currentRowChanged, stack_,
  ...
  // --- Network ---
  {
    ...
    udpPort_ = new QSpinBox;
    udpPort_->setRange(1, 65535);
    udpPort_->setValue(s.udpPort);
    f->addRow("UDP listen port:", udpPort_);
    f->addRow(new QLabel(
        "Receives QSOs broadcast by WSJT-X and similar (default 2237)."));
  }

  // --- Rig ---
  {
    auto* f = addPage("Rig");
    rigModel_ = new QSpinBox;
    rigModel_->setRange(1, 99999);
    rigModel_->setValue(s.rigModel);
    rigDevice_ = new QLineEdit(qstr(s.rigDevice));
    rigPoll_ = new QSpinBox;
    rigPoll_->setRange(50, 60000);
    rigPoll_->setValue(s.rigPollMs);
    rigAuto_ = new QCheckBox("Connect to rig at startup");
    rigAuto_->setChecked(s.rigAutoConnect);
  ...
  // --- DX Cluster ---
  {
    ...
    dxPort_ = new QSpinBox;
    dxPort_->setRange(1, 65535);
    dxPort_->setValue(s.dxPort);
    dxLogin_ = new QLineEdit(qstr(s.dxLogin));
    dxAuto_ = new QCheckBox("Connect at startup");
    dxAuto_->setChecked(s.dxAutoConnect);
  ...
  // --- LoTW ---
  {
    ...
    lotwPass_ = new QLineEdit(qstr(s.lotwPassword));
    lotwPass_->setEchoMode(QLineEdit::Password);
  ...
  // --- QRZ ---
  {
    ...
    qrzPass_ = new QLineEdit(qstr(s.qrzPassword));
    qrzPass_->setEchoMode(QLineEdit::Password);
    qrzCacheDays_ = new QSpinBox;
    qrzCacheDays_->setRange(0, 36500);
    qrzCacheDays_->setSuffix(" days");
    qrzCacheDays_->setValue(s.qrzCacheDays);
  ...
  // --- CW Keyer ---
  {
    ...
    keyerPort_ = new QSpinBox;
    keyerPort_->setRange(1, 65535);
    keyerPort_->setValue(s.keyerPort);
    keyerSpeed_ = new QSpinBox;
    keyerSpeed_->setRange(0, 60);
    keyerSpeed_->setValue(s.keyerSpeed);
    ...
    f->addRow(
        new QLabel("Tokens: %CALL% %NAME% %QTH% %RST% (from the entry form)."));
  }
  ...
  // --- Paddle ---
  {
    ...
    paddlePort_ = new QSpinBox;
    paddlePort_->setRange(1, 65535);
    paddlePort_->setValue(s.paddlePort);
    paddleWpm_ = new QSpinBox;
    paddleWpm_->setRange(1, 99);
    paddleWpm_->setValue(s.paddleWpm);
    paddleIambicB_ = new QCheckBox("Iambic B (default: iambic A)");
    paddleIambicB_->setChecked(s.paddleIambicB);
    paddleAutospace_ =
        new QCheckBox("Autospace (enforce inter-character spacing)");
    paddleAutospace_->setChecked(s.paddleAutospace);
    paddleSidetone_ = new QCheckBox("Local sidetone");
    paddleSidetone_->setChecked(s.paddleSidetone);
    paddleTone_ = new QSpinBox;
    paddleTone_->setRange(100, 2000);
    paddleTone_->setSuffix(" Hz");
    paddleTone_->setValue(s.paddleToneHz);
    paddleLevel_ = new QSpinBox;
    paddleLevel_->setRange(0, 100);
    paddleLevel_->setValue(s.paddleLevel);
    paddleDevice_ = new QLineEdit(qstr(s.paddleSidetoneDevice));
    paddleDevice_->setPlaceholderText("ALSA playback device, e.g. default");
    paddleMute_ = new QCheckBox("Mute rig audio while keying");
    paddleMute_->setChecked(s.paddleMuteAudio);
    paddleMuteTail_ = new QSpinBox;
    paddleMuteTail_->setRange(0, 5000);
    paddleMuteTail_->setSuffix(" ms");
  ...
  // --- Audio ---
  {
    ...
    audioPort_ = new QSpinBox;
    audioPort_->setRange(1, 65535);
    audioPort_->setValue(s.audioPort);
    audioRate_ = new QComboBox;
    for (int r : {8000, 12000, 16000, 24000, 48000})
      audioRate_->addItem(QString::number(r), r);
    audioRate_->setCurrentIndex(
        std::max(0, audioRate_->findData(s.audioSampleRate)));
    audioChan_ = new QSpinBox;
    audioChan_->setRange(1, 2);
    audioChan_->setValue(s.audioChannels);
  ...
  // --- Sync ---
  {
    auto* f = addPage("Sync");
    syncEnabled_ =
        new QCheckBox("Synchronise the default logbook with LAN peers");
    ...
    syncPort_ = new QSpinBox;
    syncPort_->setRange(1, 65535);
    syncPort_->setValue(s.syncPort);
    ...
    syncRequireIdentity_ =
        new QCheckBox("Reject peers without a verified identity");
    ...
    f->addRow(new QLabel(
        "Peers on the same LAN find each other automatically — no\n"
  ...
  // --- Skimmer ---
  {
    ...
    skGate_ = new QSpinBox;
    skGate_->setRange(-30, 30);
    skGate_->setSuffix(" dB");
    skGate_->setValue(s.skimmerGate);
    skMinSnr_ = new QSpinBox;
    skMinSnr_->setRange(0, 40);
    skMinSnr_->setSuffix(" dB");
    skMinSnr_->setValue(s.skimmerMinSnr);
    skKnownOnly_ = new QCheckBox("Paranoid: only surface DB-confirmed calls");
    skKnownOnly_->setChecked(s.skimmerKnownOnly);
    skBwNormDb_ = new QSpinBox;
    skBwNormDb_->setRange(0, 24);
    skBwNormDb_->setSuffix(" dB/oct");
    skBwNormDb_->setValue(s.skimmerBwNormDb);
    skBwNormRef_ = new QSpinBox;
    skBwNormRef_->setRange(100, 6000);
    skBwNormRef_->setSuffix(" Hz");
    skBwNormRef_->setValue(s.skimmerBwNormRefHz);
    skBwOffset_ = new QSpinBox;
    skBwOffset_->setRange(-30, 30);
    skBwOffset_->setSuffix(" dB");
    skBwOffset_->setValue(s.skimmerBwOffsetDb);
...
Settings QtSettingsDialog::result() const {
  ...
  for (int i = 0; i < 9; ++i) s.keyerMessages[i] = sstr(keyerMsgs_[i]);
