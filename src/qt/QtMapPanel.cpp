...
public:
    ...
    void setEndpoints(std::optional<geo::LatLon> from, std::optional<geo::LatLon> to) {
        from_ = from;
        to_ = to;
        path_.clear();
        if (from_ && to_)
...
private:
    ...
    // Draw a lat/lon polyline, breaking the stroke where it wraps the antimeridian.
...
QtMapPanel::QtMapPanel(QWidget* parent) : QWidget(parent) {
    ...
    connect(fromEdit_, &QLineEdit::textChanged, this, [this]() { if (!loading_) recompute(); });
    connect(toEdit_, &QLineEdit::textChanged, this, [this]() { if (!loading_) recompute(); });
    recompute();
}
