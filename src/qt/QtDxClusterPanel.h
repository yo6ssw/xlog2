...
// Qt equivalent of the gtkmm DxClusterPanel: a band map that aggregates
// incoming spots by (frequency, DX call) — grouping reports within +/-200 Hz of
// the same callsign — and shows one row per pair (Freq / DX / Band / spotter
// count / Entity / Cont) ordered by frequency, with band-filter chips, a
// connect/command bar and a raw telnet console. Each spotter expires 5 minutes
// after its report; an entry disappears once its last spotter has expired.
class QtDxClusterPanel : public QWidget {
 ...
 private:
  ...
  void applyMutable(int row, const Entry& e,
                    Clock::time_point now);  // count + tooltip
  void onSend();
