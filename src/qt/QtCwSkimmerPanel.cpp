...
// colour stops are packed into the upper range (most of the travel happens
// above 0.55) so differences in power among strong signals read as clear colour
// steps, not just "bright vs brighter". A gamma >1 first dims the low end
// (noise).
QRgb heat(float v) {
...
class SkimmerWaterfall : public QWidget {
 ...
 private:
  ...
  void scrollOne(const std::vector<float>& mags) {
    ...
    for (int x = 0; x < cols; ++x) top[x] = heat(mags[x]);
  }

  // Steady-cadence consumer: one row per tick normally; advance a couple when a
  // burst has piled up so the backlog drains without a visible multi-pixel
  // jump.
  void drain() {
 ...
 protected:
  void paintEvent(QPaintEvent*) override {
    ...
    if (!img_.isNull()) p.drawImage(rect(), img_, img_.rect());
    ...
    for (const auto& [hz, call] : labels_) {
      ...
      const int x = static_cast<int>((hz - minHz_) / span * width());
      const QRect box(x + 2, 1, p.fontMetrics().horizontalAdvance(call) + 6,
                      15);
...
QtCwSkimmerPanel::QtCwSkimmerPanel(QWidget* parent) : QWidget(parent) {
  ...
  // Detection-gate slider: higher = stronger signals only (suppress
  // noise/ghosts).
...
void QtCwSkimmerPanel::showChannelText(int id) {
  ...
  connect(dlg, &QObject::destroyed, this, [this, id, edit] {
    ...
    if (auto it = textViews_.find(id);
        it != textViews_.end() && it->second == edit)
      textViews_.erase(it);
  });
  dlg->show();
}
...
void QtCwSkimmerPanel::setCallDbInfo(bool loaded, std::size_t count) {
  ...
  knownOnly_->setText(
      loaded ? QString("Show only calls in database (%1)").arg(count)
             : QString("Show only calls in database (none loaded)"));
}

void QtCwSkimmerPanel::addWaterfall(const std::vector<float>& mags,
...
void QtCwSkimmerPanel::updateChannel(int id, double hz, int wpm,
  ...
  const QString freq =
      QString::number(static_cast<int>(std::lround(hz))) + " Hz";
  ...
  if (it == rows_.end()) {
    ...
    QList<QStandardItem*> cells = {
        freqItem, new QStandardItem(QString::number(wpm)),
    ...
    for (int r = 0; r < model_->rowCount(); ++r)
      if (model_->item(r, 0)->data(Qt::UserRole).toDouble() > hz) {
        pos = r;
        break;
      }
