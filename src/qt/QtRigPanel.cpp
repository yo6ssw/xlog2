...
QString formatFreq(double mhz) {
  if (mhz <= 0.0) return QStringLiteral("—.———.——");
...
QtRigPanel::QtRigPanel(QWidget* parent) : QWidget(parent) {
  ...
  connect(powerButton_, &QPushButton::clicked, this, [this](bool checked) {
    if (!updatingPower_) emit setPower(checked);
  });
  ...
  for (const char* m : kModes) modeCombo_->addItem(QString::fromUtf8(m));
  connect(modeCombo_, qOverload<int>(&QComboBox::activated), this,
          [this](int idx) {
            if (!updatingMode_ && idx >= 0 &&
                idx < static_cast<int>(std::size(kModes)))
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    connect(t, &QPushButton::clicked, this, [this, n]() {
      if (!updatingFilter_) emit setFilter(n);
    });
    filterButtons_[i] = t;
  ...
  connect(agcButton_, &QPushButton::clicked, this,
          [this](bool checked) { emit setAgc(checked); });
...
void QtRigPanel::updateFilterButtons(int filter) {
  ...
  for (int i = 0; i < 3; ++i)
    if (filterButtons_[i]) filterButtons_[i]->setChecked(filter == i + 1);
  updatingFilter_ = false;
}

void QtRigPanel::setState(double mhz, const std::string& mode, int pbwidthHz,
                          int filter) {
  freqLabel_->setText(formatFreq(mhz));

  updateModeCombo(mode);
  modeLabel_->setText(pbwidthHz > 0 ? QString::number(pbwidthHz) + " Hz"
                                    : QString());
...
void QtRigPanel::setConnected(bool connected) {
  ...
  if (!connected) {
    ...
    if (powerButton_) powerButton_->setVisible(false);
  }
}
