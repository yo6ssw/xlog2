...
class QtMainWindow : public QMainWindow, public IMainView {
 ...
 private:
  ...
  void attachSyncedLog(
      QtLogPage* page);        // bind the coordinator to the default log
  ...
  void onSkimmerDock(
      const std::string& side);             // move the skimmer dock to a side
  ...
  void
  restoreDockSize();  // apply the saved DX-cluster dock size once, post-show
  ...
  LogbookSync sync_;
  SyncCoordinator
      coordinator_;  // declared after sync_ (holds a reference to it)
  ...
  QActionGroup* dxDockGroup_ =
      nullptr;  // Cluster ▸ Dock radio (top/bottom/left/right)
