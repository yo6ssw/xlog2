...
public:
    ...
    void applyDxSpot(const std::string& c, double mhz) { presenter_.applyDxSpot(c, mhz); }
    void backfillDxcc()                          { presenter_.backfillDxcc(); }

    std::vector<Qso> qsosNotLotwSent() const { return presenter_.qsosNotLotwSent(); }
...
private:
    ...
    // widgets (which fire change signals that would call back into the presenter).
