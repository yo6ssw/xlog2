...
MainWindow::MainWindow()
  ...
  signal_close_request().connect(
      sigc::mem_fun(*this, &MainWindow::onCloseRequest), false);
  ...
  for (int i = 0; i < 9; ++i) {
    auto action = Gtk::CallbackAction::create(
        [this, i](Gtk::Widget&, const Glib::VariantBase&) {
          if (auto* page = currentPage()) page->presenter().onSendCwClicked(i);
          return true;
        });
    keyerShortcuts->add_shortcut(Gtk::Shortcut::create(
        Gtk::KeyvalTrigger::create(GDK_KEY_F1 + i), action));
  }
  add_controller(keyerShortcuts);

  // Paddle simulation for the remote paddle keyer: `[` = dit, `]` = dah, held
  // for the duration of the contact. Only intercepted while the keyer is
  // active, so the brackets type normally otherwise. CAPTURE phase so the
  // press/release is seen before any focused entry consumes it.
  ...
  paddleKeys->signal_key_pressed().connect(
      [this](guint keyval, guint, Gdk::ModifierType) -> bool {
        if (!paddle_.isActive()) return false;
  ...
  notebook_.signal_switch_page().connect([this](Gtk::Widget* page, guint) {
    if (auto* lp = dynamic_cast<LogPage*>(page)) {
      set_title(Glib::ustring("xlog2 ") + xlog::kVersion + " — " + lp->title() +
  ...
  rigPanel_.signalStep().connect([this](double hz) {
    if (rig_.isRunning()) rig_.stepFrequency(hz);
  });
  rigPanel_.signalSetFilter().connect([this](int n) {
    if (rig_.isRunning()) rig_.setFilter(n);
  });
  rigPanel_.signalSetPower().connect([this](bool on) {
    if (rig_.isRunning()) rig_.setPower(on);
  });
  rigPanel_.signalSetAgc().connect([this](bool on) {
    if (rig_.isRunning()) rig_.setAgc(on);
  });
  rigPanel_.signalSetMode().connect([this](std::string mode) {
    if (rig_.isRunning()) rig_.setMode(mode);
  });
  ...
  // $XDG_DATA_HOME/xlog2/master.scp.
  {
    const std::size_t n = skimmer_.loadCallsignDb(
        Glib::build_filename(Glib::get_user_data_dir(), "xlog2", "master.scp"));
  ...
  // needed).
  {
    ...
    qrz_.setCache(Glib::build_filename(dir, "qrz-cache.sqlite"),
                  cfg().qrzCacheDays);
  }
  if (notebook_.get_n_pages() == 0) openDefaultLog();
...
void MainWindow::buildActions() {
  ...
  add_action("filldxcc", [this]() {
    if (auto* page = currentPage()) page->backfillDxcc();
  });
  ...
  udpAction_ = add_action_bool(
      "udp", sigc::mem_fun(*this, &MainWindow::onToggleUdp), false);

  add_action("rigconnect", sigc::mem_fun(*this, &MainWindow::onRigConnect));
  add_action("rigdisconnect",
             sigc::mem_fun(*this, &MainWindow::onRigDisconnect));
  ...
  paddleAction_ = add_action_bool(
      "paddle", sigc::mem_fun(*this, &MainWindow::onTogglePaddle), false);

  audioAction_ = add_action_bool(
      "audio", sigc::mem_fun(*this, &MainWindow::onToggleAudio), false);
  ...
  skimmerShowAction_ = add_action_bool(
      "skimmershow", sigc::mem_fun(*this, &MainWindow::onSkimmerToggleShow),
      false);
  ...
  add_action("synctrust",
             sigc::mem_fun(*this, &MainWindow::onManageTrustedPeers));
  syncEnableAction_ = add_action_bool(
      "syncenabled",
      [this]() {
...
void MainWindow::registerTab(LogPage* page) {
  ...
  page->signalStatus().connect(
      [this](const Glib::ustring& m) { setStatus(m.raw()); });
  ...
  page->signalSendCw().connect([this](const std::string& text) {
    if (!keyer_.send(text)) setStatus("Keyer: " + keyer_.lastError());
  });
  page->signalAbortCw().connect([this]() {
    if (!keyer_.abort()) setStatus("Keyer: " + keyer_.lastError());
  });
...
void MainWindow::onPageChanged(LogPage* page) {
  updateTabLabel(page);
  if (page == currentPage()) updateTitle();
}
...
LogPage* MainWindow::findPageByPath(const std::string& path) {
  for (int i = 0; i < notebook_.get_n_pages(); ++i)
    if (auto* p = dynamic_cast<LogPage*>(notebook_.get_nth_page(i)))
      if (p->isFileBacked() && p->path() == path) return p;
  return nullptr;
}
...
void MainWindow::onOpen() {
  ...
  dialog->set_filters(
      ui::makeFilters("xlog2 logbook", {"*.xlog", "*.db", "*.sqlite"}));
  dialog->open(*this,
...
void MainWindow::onSaveAs() {
  ...
  dialog->save(*this, [this, dialog,
                       page](const Glib::RefPtr<Gio::AsyncResult>& result) {
    try {
...
void MainWindow::onCloseTab() {
  if (auto* page = currentPage()) closePage(page);
}
...
void MainWindow::onImportAdif() {
  ...
  dialog->set_filters(
      ui::makeFilters("ADIF files", {"*.adi", "*.adif", "*.ADI"}));
  dialog->open(*this, [this, dialog,
                       page](const Glib::RefPtr<Gio::AsyncResult>& result) {
    try {
      auto file = dialog->open_finish(result);
      if (!file) return;
...
void MainWindow::onExportAdif() {
  ...
  dialog->save(*this, [this, dialog,
    ...
    try {
      ...
      setStatus("Exported " + std::to_string(page->qsoCount()) +
                " QSO(s) to ADIF.");
    } catch (const Glib::Error& e) {
...
void MainWindow::onFind() {
  if (auto* page = currentPage()) page->beginSearch();
}
...
void MainWindow::startUdpListening() {
  ...
  if (cfg().udpEnabled)
    setStatus("Listening for QSOs on UDP port " +
              std::to_string(cfg().udpPort) + " (WSJT-X / ADIF).");
  else
...
void MainWindow::onQrzLookup(LogPage* page, const std::string& callsign) {
  if (cfg().qrzUser.empty() || cfg().qrzPassword.empty()) {
    setStatus(
        "Set your QRZ.com username and password in QRZ ▸ Settings first.");
    return;
  }
  if (qrz_.isBusy()) {
...
void MainWindow::applyDxConfig() {
  if (cfg().dxDock != "top" && cfg().dxDock != "bottom" &&
    ...
    cfg().dxDock = "bottom";
  if (dxDockAction_)
    dxDockAction_->set_state(
        Glib::Variant<Glib::ustring>::create(cfg().dxDock));
  if (dxShowAction_)
  ...
  if (cfg().dxPanelPos > 0) paned_.set_position(cfg().dxPanelPos);
...
void MainWindow::applyRigConfig() {
  if (cfg().rigDock != "top" && cfg().rigDock != "bottom" &&
    ...
    cfg().rigDock = "right";
  if (rigDockAction_)
    rigDockAction_->set_state(
        Glib::Variant<Glib::ustring>::create(cfg().rigDock));
  if (rigShowAction_)
    rigShowAction_->set_state(Glib::Variant<bool>::create(cfg().rigVisible));
  applyRigDock();
  if (cfg().rigPanelPos > 0) rigPaned_.set_position(cfg().rigPanelPos);
}
...
void MainWindow::applySkimmerDock() {
  ...
  const bool panelFirst =
      (cfg().skimmerDock == "top" || cfg().skimmerDock == "left");
  if (panelFirst) {
...
void MainWindow::applySkimmerConfig() {
  ...
  if (skimmerDockAction_)
    skimmerDockAction_->set_state(
        Glib::Variant<Glib::ustring>::create(cfg().skimmerDock));
  if (skimmerShowAction_)
    skimmerShowAction_->set_state(
        Glib::Variant<bool>::create(cfg().skimmerVisible));
  applySkimmerDock();
  if (cfg().skimmerPanelPos > 0)
    skimmerPaned_.set_position(cfg().skimmerPanelPos);
  // Restore the gate + min-SNR levels (slider + service) before the skimmer
  // starts.
  skimmerPanel_.setGate(cfg().skimmerGate);
  skimmer_.setGate(static_cast<float>(cfg().skimmerGate));
  skimmerPanel_.setMinSnr(cfg().skimmerMinSnr);
  ...
  skimmer_.setBandwidthNorm(cfg().skimmerBwNormDb, cfg().skimmerBwNormRefHz,
                            cfg().skimmerBwOffsetDb);
  if (cfg().skimmerVisible) startSkimmer();
}
...
void MainWindow::onSkimmerDock(const Glib::ustring& side) {
  ...
  if (!cfg()
           .skimmerVisible) {  // picking a dock implies wanting the panel shown
...
void MainWindow::applyMapConfig() {
  if (cfg().mapDock != "top" && cfg().mapDock != "bottom" &&
    ...
    cfg().mapDock = "right";
  if (mapDockAction_)
    mapDockAction_->set_state(
        Glib::Variant<Glib::ustring>::create(cfg().mapDock));
  if (mapShowAction_)
    mapShowAction_->set_state(Glib::Variant<bool>::create(cfg().mapVisible));
  applyMapDock();
  if (cfg().mapPanelPos > 0) mapPaned_.set_position(cfg().mapPanelPos);
...
void MainWindow::startSync() {
  if (syncEnableAction_)
    syncEnableAction_->set_state(
        Glib::Variant<bool>::create(cfg().syncEnabled));
  if (!cfg().syncEnabled) {
    sync_.stop();
    updateSyncIndicator();
    return;
  ...
  if (!cfg().syncSecret.empty()) {
    c.identityFile = Glib::build_filename(Glib::get_user_data_dir(), "xlog2",
                                          "node_identity");
...
void MainWindow::onSyncNow() { coordinator_.syncNow(); }
...
void MainWindow::onManageTrustedPeers() {
  ...
  keyEntry->set_text(key.empty() ? "(identity disabled — set a shared secret)"
                                 : key);
  ...
  copy->signal_clicked().connect(
      [this, key]() { get_clipboard()->set_text(key); });
  keyRow->append(*copy);
  box->append(*keyRow);
...
std::string MainWindow::layoutFilePath() const {
  return Glib::build_filename(Glib::get_user_config_dir(), "xlog2",
                              "layout.ini");
}
...
void MainWindow::saveSettings() {
  ...
  if (auto* page = currentPage()) page->storeColumnLayout(ini);
...
void MainWindow::loadSettings() {
  ...
  if (loaded && settings_.hasGroup("window")) {
    if (settings_.hasKey("window", "width") &&
        settings_.hasKey("window", "height")) {
      const int w = settings_.getInt("window", "width", 0);
      const int h = settings_.getInt("window", "height", 0);
      if (w > 0 && h > 0) set_default_size(w, h);
    }
    if (settings_.getBool("window", "maximized", false)) maximize();
  }
  ...
  for (const auto& path : open) {
    auto* page = addPage(Gtk::make_managed<LogPage>());
    if (!page->openFile(path)) closePage(page);
  }
  if (!active.empty())
