...
LogPage::LogPage() : Gtk::Box(Gtk::Orientation::VERTICAL), presenter_(*this) {
  ...
  presenter_.onLookupCall = [this](const std::string& c) {
    signalLookupCall_.emit(c);
  };
  presenter_.onSendCw = [this](const std::string& t) { signalSendCw_.emit(t); };
  presenter_.onAbortCw = [this]() { signalAbortCw_.emit(); };
  presenter_.onLocator = [this](const std::string& g) {
    signalLocator_.emit(g);
  };
...
Glib::RefPtr<Gtk::ColumnViewColumn> LogPage::makeColumn(
  ...
  factory->signal_setup().connect(
      [this](const Glib::RefPtr<Gtk::ListItem>& li) {
        ...
        // Secondary-button click anywhere in the row opens the row context
        // menu. The ListItem outlives its child label, so a raw pointer is
        // safe; its get_item() yields whichever QSO is currently bound to this
        // row slot.
  ...
  factory->signal_bind().connect(
      [getter](const Glib::RefPtr<Gtk::ListItem>& li) {
        ...
        if (label && item) label->set_text(getter(item->qso));
      });
...
void LogPage::buildLogView() {
  ...
  filter_ =
      Gtk::StringFilter::create(Gtk::ClosureExpression<Glib::ustring>::create(
  ...
  auto add = [&](const std::string& id, const Glib::ustring& title,
                 std::function<std::string(const Qso&)> getter,
                 bool expand = false) {
  ...
  filler_ =
      Gtk::ColumnViewColumn::create("", Gtk::SignalListItemFactory::create());
  ...
  move->signal_activate().connect([this](const Glib::VariantBase& p) {
    const int idx = std::stoi(
        Glib::VariantBase::cast_dynamic<Glib::Variant<Glib::ustring>>(p)
            .get()
            .raw());
...
void LogPage::showRowContextMenu(Gtk::ListItem* li, Gtk::Widget& anchor,
  ...
  if (gtk_widget_compute_point(anchor.gobj(), columnView_.Gtk::Widget::gobj(),
    ...
    rowMenu_->set_pointing_to(
        Gdk::Rectangle(static_cast<int>(out.x), static_cast<int>(out.y), 1, 1));
  rowMenu_->popup();
}
...
void LogPage::confirmDeleteRow(long id) {
  ...
  if (!q) return;
  const Glib::ustring call =
      q->call.empty() ? Glib::ustring("this QSO") : Glib::ustring(q->call);
  ...
  if (!win) return;
  dialog->choose(*win,
                 [this, dialog, id](const Glib::RefPtr<Gio::AsyncResult>& res) {
                   try {
...
void LogPage::moveColumn(const Glib::ustring& id, int delta) {
  ...
  int pos = -1;
  for (int i = 0; i < n; ++i)
    if (model->get_object(i).get() == col.get()) {
      pos = i;
      break;
    }
  if (pos < 0) return;
  ...
  if (target < 0 || target >= n) return;  // already at an edge
...
void LogPage::showAllColumns() {
  for (const auto& [cid, c] : columns_) c->set_visible(true);
}
...
void LogPage::buildSearch() {
  searchEntry_.set_placeholder_text(
      "Search log — call, name, QTH, locator, comment, band, mode…");
...
Glib::ustring LogPage::rowSearchText(
    const Glib::RefPtr<Glib::ObjectBase>& obj) {
...
void LogPage::onSearchChanged() {
  ...
  if (query.empty())
    ...
    status("Search: " + std::to_string(filterModel_->get_n_items()) + " of " +
           std::to_string(store_->get_n_items()) + " QSOs match \"" + query +
           "\".");
}

void LogPage::beginSearch() { showSearch(); }
...
void LogPage::buildEntryForm() {
  ...
  std::vector<Glib::ustring> bandStrings(bands::names().begin(),
                                         bands::names().end());
  std::vector<Glib::ustring> modeStrings(bands::modes().begin(),
                                         bands::modes().end());
  ...
  call_.set_icon_from_icon_name("edit-find-symbolic",
  ...
  freq_.signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onFreqChanged();
  });
  call_.signal_activate().connect([this]() { presenter_.onAddOrUpdate(); });

  // Live dupe detection + DXCC lookup as the key fields change.
  call_.signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onCallChanged();
  });
  date_.signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  band_.property_selected().signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  mode_.property_selected().signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onDupeKeyChanged();
  });
  locator_.signal_changed().connect([this]() {
    if (!loadingForm_) presenter_.onLocatorChanged();
  });
}

void LogPage::onSelectionChanged() {
  auto* item = dynamic_cast<QsoItem*>(selection_->get_selected_item().get());
  if (!item) return;  // deselection — leave the form alone
...
void LogPage::setRows(const std::vector<Qso>& qsos) {
  store_->remove_all();
  for (const auto& q : qsos) store_->append(QsoItem::create(q));
}

void LogPage::clearSelection() { selection_->unselect_all(); }
...
void LogPage::setBand(const std::string& s) {
  ui::setDropdown(band_, bandModel_, s);
}
void LogPage::setMode(const std::string& s) {
  ui::setDropdown(mode_, modeModel_, s);
}
...
void LogPage::setDupeWarning(const std::string& msg, bool highlight) {
  ...
  if (highlight) {
    if (!call_.has_css_class("dupe")) call_.add_css_class("dupe");
  } else {
    call_.remove_css_class("dupe");
  }
}

void LogPage::setDxccText(const std::string& s) { dxccLabel_.set_text(s); }
...
void LogPage::focusCall() { call_.grab_focus(); }
...
void LogPage::storeColumnLayout(IniFile& ini) {
  ...
  for (guint i = 0; i < displayed->get_n_items(); ++i) {
    auto* colPtr =
        dynamic_cast<Gtk::ColumnViewColumn*>(displayed->get_object(i).get());
    for (const auto& [id, col] : columns_) {
      if (col.get() == colPtr) {
  ...
  for (const auto& [id, col] : columns_) {
    ...
    if (width > 0) ini.setInt("width", id, width);
...
void LogPage::applyColumnOrder(const std::vector<std::string>& ids) {
  ...
  for (const auto& [cid, col] : columns_)
    if (!used.count(cid)) desired.push_back(col);

  if (desired.size() != columns_.size()) return;

  for (const auto& [cid, col] : columns_) columnView_.remove_column(col);
  for (const auto& col : desired) columnView_.append_column(col);
